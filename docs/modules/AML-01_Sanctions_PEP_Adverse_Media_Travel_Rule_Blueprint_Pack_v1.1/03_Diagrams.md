...
## 6. Pre-Transaction Sanctions Gate

```mermaid
flowchart TD
  A[Payment / Settlement / Transfer Request] --> B[AML-01 Pre-Transaction Gate]
  B --> C{Input Complete + List Fresh?}
  C -->|No| D[Hold / Deny]
  C -->|Yes| E[Screen Parties + Counterparties + UBO Graph]
  E --> F{Hit?}
  F -->|True Hit| G[Block + Escalate]
  F -->|Possible| H[Hold / Review]
  F -->|Clear| I[Short-Lived Action-Scoped Clear]
```

---

## 7. List Update Interim Blocking

```mermaid
flowchart TD
  A[List Update] --> B[Identify Affected Parties]
  B --> C[Apply Interim Block]
  C --> D[Rescreen Within SLA]
  D --> E{Clear?}
  E -->|Yes| F[Controlled Release]
  E -->|No| G[Restrict / Escalate]
```

---

## 8. Ownership-Based 50 Percent Rule

```mermaid
flowchart TD
  A[KYC-01 UBO Graph] --> B[Screen UBOs / Controllers]
  B --> C[Aggregate Sanctioned Ownership]
  C --> D{>= 50% or Control?}
  D -->|Yes| E[Entity Restricted / Hit]
  D -->|No| F[Continue Screening]
```
...
```mermaid
flowchart TD
  A[Suspicion Formed] --> B[Start Internal Clock]
  B --> C[MLRO Review Deadline]
  C --> D[FIU Filing Deadline]
  D --> E{Decision}
  E -->|File| F[STR Filing Workflow]
  E -->|Not File| G[Reasoned Closure]
  B --> H[No Tipping-Off Transaction Handling]
```
