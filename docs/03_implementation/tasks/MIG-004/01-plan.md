...
- **Baseline commit:** `6d2e4fe` (MIG-005 accepted at `5f78a0b`)
- **Human approval:** **PENDING.** Approval must also confirm decisions HD-1…HD-4 (§14).
...
- Doc 00 §25.2 also says "CFG-01 owns … `PRODUCTION_ACTIVATION_STATE` (`MIG-004`)". The §25.3 register row, STR-03 and §1.D's owner column all confine MIG-004 to state 2. This plan follows §25.3: **state 3 is not built here** (§11, HD-2).
...
3. **`evaluate`:** if `body.environment !== own` →
   ...
   The check runs immediately after decision-time integrity verification, **before step 1**, so no other step evaluates a mismatched request.
4. **`verify-decision`:**
...
**Resulting order in code:** integrity → environment-mismatch → step 1 prohibited (registry + structural `exchange.` prefix, HD-4) → step 6 kill switch → feature lookup (unknown → step 12 deny) → **step 4 environment** → step 10 stale → step 8 state → **production hold (HD-2)** → allow. All steps deny, so order only affects which reason is reported. Prohibited stays first (Q9).
...
- **PRODUCTION_ACTIVATION_STATE (state 3) is not faithfully representable in the current schema.** No five-valued state exists, and the `CFG-RULE-004` activation path (three checkers, §21 evidence) does not exist.
  ...
  - **HD-2 (recommended):** under `SYS-RULE-010` an absent state denies, so in canonical PRODUCTION (`prod` and `staging`) a request that passes every other step is **denied with `production_activation_unrepresented`** until a later migration builds state 3.
...
|---|---|
| Feature scope seal hash | `computeFeatureScopeHash` adds `environment_scope` to its allow-list, so the next governed reseal covers it |
| Decision-time seal verification | **Pre-existing gap:** the `feature` seal is written by `resealScope` but **never verified** (`verifyDecisionTimeIntegrity` checks `licence_profile` and `prohibited_registry` only), so out-of-band writes to `current_state` are undetected today, and would be for `environment_scope`. **HD-3** decides whether to fix it here or separately |
| `feature_version.state_snapshot` | Snapshot becomes `{current_state, environment_scope}` (read from the row in the same transaction) |
...
| Fresh re-check at verify | **Added:** verify-decision re-reads the feature's `environment_scope` for its own canonical environment, like the kill switch. Anything other than `ENABLED` → revoke `environment_unavailable` + `CFG1_DECISION_BINDING_MISMATCH`. This invalidates outstanding tokens even after an out-of-band change that did not bump the version |
...
- Unchanged denials still hold:
  ...
  - canonical PRODUCTION, all else passing → `production_activation_unrepresented` (HD-2).
- DEVELOPMENT-configured, entry `ENABLED`, `current_state = 'enabled'` → allow and a token (proves the non-production path works).
- `feature_version.state_snapshot` carries `environment_scope`; the feature seal hash changes when `environment_scope` changes.
...
## 14. Human decisions required at approval
| ID | Decision | Recommendation |
|---|---|---|
| **HD-1** | `current_state` = "product activation" conjunct (every environment), not state 2 and not state 3 | **Approve.** Record in `DECISION_LOG.md` (`DEC-014`) in the implementation turn or a paired docs turn |
| **HD-2** | PRODUCTION hold: canonical PRODUCTION denies `production_activation_unrepresented` until state 3 exists | **Approve.** `SYS-RULE-010`; zero impact today; makes every production/staging allow depend on a state-3 migration |
| **HD-3** | Decision-time verification of the `feature` seal | **Separate task** (proposed `CFG-FIND-002`) **before any UAT of environment-gated capabilities**. Adding it here needs a reseal-in-migration design; MIG-004's fresh verify re-check (§10) covers tokens meanwhile |
| **HD-4** | Structural `exchange.` prefix deny at evaluate step 1 | **Approve.** Deny-only, answers Q9 directly |
...
## 17. New review-required issues (recorded here, not registered)
- **R-1:** CFG-01 blueprint v1.1 allows a synthetic non-production exception to the prohibited lock, which conflicts with Doc 00 v1.5 §1.D.4 / `LIC-RULE-002`. The masters prevail. The blueprint needs a rebaseline (Doc 00 §25.2 lists CFG-01 as "Extension").
- **R-2:** blueprint FR-016 lists environments "dev, staging, UAT, production". Stale.
- **R-3:** the `feature` seal is never verified at decision time (HD-3, proposed `CFG-FIND-002`). Pre-existing.
- **R-4:** the feature-changes workflow can set `current_state = 'enabled'` in a production deployment through a generic IAM-02 approval, which is not the `CFG-RULE-004` path. Pre-existing; HD-2's hold neutralises it until state 3 exists.
- **R-5:** Doc 00 §25.2's "`PRODUCTION_ACTIVATION_STATE` (`MIG-004`)" wording versus the §25.3 register. Add it to governance cleanup G-1.
...
- [ ] Authoritative own environment everywhere per §5; the mismatch denies, logs, audits critical and never issues a token; cross-environment tokens fail.
