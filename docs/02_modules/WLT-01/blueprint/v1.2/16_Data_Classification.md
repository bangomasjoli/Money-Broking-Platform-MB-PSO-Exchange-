...
| Data | Classification | Protection |
|---|---|---|
| Destination revocation epoch | Security Critical | Versioned/audited |
| Limit profile/evaluation | Restricted / Security Critical | Approval/audit |
| Inbound source screening | Restricted / AML Sensitive | Quarantine control |
| Deposit quarantine state | Security Critical | Ledger/deposit gating |
| Proof-of-control challenge/result | Restricted / Security Critical | Expiring evidence |
| Address integrity check | Security Critical | Chain-specific audit |
| Chain/provider coverage | Restricted / Integrity Evidence | Approval-controlled |
| AML revocation signal | Security Critical | Immediate propagation |
| Name-service alias hash | Restricted | Not authoritative |
| Address poisoning indicator | Restricted / AML Sensitive | Review controlled |

Rules:

1. Name-service aliases are not authoritative destinations.
2. Inbound source data is minimised and scoped.
3. Quarantine status is security-critical and must not be bypassed.
4. Limit evaluation data is audit evidence.
5. Proof-of-control data expires and is access controlled.
