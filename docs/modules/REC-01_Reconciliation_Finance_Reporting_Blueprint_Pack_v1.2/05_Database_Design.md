...
| population_coverage_status | varchar | pass/fail/partial/not_required |
| as_of_anchor_type | varchar | led_journal_sequence/e2e_saga_sequence |
| as_of_anchor_value | varchar | Shared cut-off |
| rec_hash_chain_prev | varchar | Previous hash |
| rec_hash_chain_current | varchar | Current hash |
| external_anchor_ref | varchar | SEC/evidence anchor |
...
| as_of_anchor_value | varchar | Shared cut-off |
| sequence_start | varchar | Monotonic start |
| sequence_end | varchar | Monotonic end |
| sequence_gap_status | varchar | pass/fail/not_applicable |
...
| break_signature | varchar | Recurrence signature |
| recurrence_count | int | Count |
| in_flight_status | varchar | not_in_flight/expected_open/sla_breached |
| owner | varchar | Owner |
...
| validation_clean_status | varchar | pass/fail/not_required |
| false_positive_independent_signoff_ref | varchar | Required for false-positive |
...
### 2.12 `rec1.population_coverage`

| Column | Type | Notes |
|---|---|---|
| id | uuid | Primary |
| population_coverage_id | varchar | Unique |
| run_id | varchar | Run |
| denominator_source | varchar | sec_manifest/e2e_registry/module_sequence |
| expected_count | int | Expected |
| observed_count | int | Observed |
| missing_count | int | Missing |
| coverage_status | varchar | pass/fail/partial |
| sec_audit_ref | varchar | Audit |

### 2.13 `rec1.in_flight_reconciling_item`
...
| expected_state | varchar | Expected-open state |
| sla_due_utc | timestamptz | SLA |
| status | varchar | open/carried_forward/converted_to_break/closed |
| sec_audit_ref | varchar | Audit |

### 2.14 `rec1.external_statement`

| Column | Type | Notes |
|---|---|---|
| id | uuid | Primary |
| statement_id | varchar | Unique |
| provider_id | varchar | Provider |
| account_ref | varchar | Safeguarding account |
| statement_type | varchar | bank/custodian/chain/lp/payment_provider |
| as_of_utc | timestamptz | As-of |
| period_start_utc | timestamptz | Start |
| period_end_utc | timestamptz | End |
| auth_status | varchar | pass/fail/manual_exception |
| completeness_status | varchar | complete/partial/missing_account |
| freshness_status | varchar | fresh/stale/misaligned |
| ingestion_sod_status | varchar | pass/fail |
| statement_hash | varchar | Hash |
| sec_audit_ref | varchar | Audit |

### 2.15 `rec1.rec_integrity_anchor`

| Column | Type | Notes |
|---|---|---|
| id | uuid | Primary |
| anchor_id | varchar | Unique |
| object_type | varchar | run/break/report/evidence_pack/close |
| object_id | varchar | Object |
| prev_hash | varchar | Previous |
| current_hash | varchar | Current |
| external_anchor_ref | varchar | SEC/evidence anchor |
| anchored_at_utc | timestamptz | Time |
| sec_audit_ref | varchar | Audit |

### 2.16 `rec1.reconciliation_policy_version`

| Column | Type | Notes |
|---|---|---|
| id | uuid | Primary |
| policy_version_id | varchar | Unique |
| policy_type | varchar | tolerance/severity/rule_set/materiality |
| version | varchar | Version |
| status | varchar | draft/approved/retired |
| effective_from_utc | timestamptz | Effective |
| approval_ref | varchar | Maker-checker |
| policy_hash | varchar | Hash |
| sec_audit_ref | varchar | Audit |

### 2.17 `rec1.report_restatement`

| Column | Type | Notes |
|---|---|---|
| id | uuid | Primary |
| restatement_id | varchar | Unique |
| original_report_id | varchar | Original |
| restated_report_id | varchar | New |
| reason_code | varchar | Reason |
| approval_ref | varchar | Approval |
| notification_status | varchar | not_required/pending/sent |
| sec_audit_ref | varchar | Audit |

### 2.18 `rec1.regulatory_obligation`

| Column | Type | Notes |
|---|---|---|
| id | uuid | Primary |
| obligation_id | varchar | Unique |
| obligation_type | varchar | safeguarding_return/audit_pack/management_report/regulatory_submission |
| regulator_or_recipient | varchar | Recipient |
| reporting_period_start | date | Start |
| reporting_period_end | date | End |
| due_date | date | Due |
| required_content | jsonb | Content |
| status | varchar | scheduled/in_progress/ready/submitted/late/waived |
| submission_hash | varchar | Submitted pack |
...
13. `population_coverage(run_id, coverage_status)`.
14. `in_flight_reconciling_item(run_id, status, sla_due_utc)`.
15. `external_statement(provider_id, account_ref, as_of_utc)`.
16. `rec_integrity_anchor(object_type, object_id)`.
17. `reconciliation_policy_version(policy_type, version, status)`.
18. `report_restatement(original_report_id, restated_report_id)`.
19. `regulatory_obligation(due_date, status)`.
...
11. Full run requires population coverage record.
12. Cross-module run requires shared as-of anchor.
13. External statements must pass auth/completeness/freshness/SoD for final safeguarding report.
14. REC run/break/report/evidence pack requires hash-chain anchor.
15. High/critical break closure requires clean validation run.
16. Recurring break requires escalation.
17. Tolerance/severity/rule policy changes require approved policy version.
18. Final report restatement creates new report, never silent edit.
19. Regulatory obligation status must track due/submitted/late.
