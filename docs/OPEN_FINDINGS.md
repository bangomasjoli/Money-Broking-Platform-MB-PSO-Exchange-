...
069 (`DECISION_LOG.md` DEC-009). Satisfies the numeric-policy prerequisite for WLT-01
BLOCKER-2 (see WLT-FIND-004) — the engine itself (migrations `068`/`069`) is not yet
implemented.

| Finding ID | Module | Control | Severity / Status | Finding | Required Action | Due / Trigger | Source / Acceptance | State |
|---|---|---|---|---|---|---|---|---|
| FND-FIND-001 | FND-01 | Shared rate-limit engine — pre-authentication abuse | HIGH | Unauthenticated garbage-bearer-token requests can reach IAM-01 session introspection (one HTTP call + one IAM DB query per attempt) before any identity-based WLT-01 rate-limit check can apply — no safe bounded-cardinality subject exists pre-authentication (a bearer-token hash or request fingerprint is attacker-chosen), and no per-IP control exists (`trustProxy` is configured nowhere in this platform; `request.ip` is the raw socket peer). A genuine amplification vector against IAM-01 once WLT-01 public routes are internet-exposed. **NOT closed by DEC-009's approved numeric policy** — that policy is authenticated-only by design; inventing a fake `AUTH_FAILURE` bucket or bolting on unsafe per-IP limiting was explicitly evaluated and rejected. | Resolve via public-perimeter / pre-authentication-abuse infrastructure (deployment-level — e.g. a gateway or a documented trusted-proxy topology enabling safe per-IP control). Mandatory precondition before any WLT-01 public route is internet-exposed. | Trigger: before any WLT-01 public route is internet-exposed | Shared Rate-Limit Engine architecture review (FINDING-2); reaffirmed by the numeric-policy governance review | OPEN |
...
| WLT-FIND-004 | WLT-01 | Client-facing public `/wlt1/*` surface | OPEN — final genuine WLT-01 control remaining | The client-facing public `/wlt1/*` surface (6-route contract, FROZEN PENDING PREREQUISITES) is gated on two blockers. **BLOCKER-1 (IAM-01 internal session-introspection seam) is now SATISFIED** — `POST /internal/auth/session/validate`, COMPLETE/ACCEPTED, commits `5794ffe`/`5a29559` (see IAM1-FIND-001…007 and `02_modules/IAM-01/acceptance/IAM-01_Session_Introspection_Opus_v1.0.md`). **BLOCKER-2 (shared rate-limit engine) REMAINS OPEN** — architecture ACCEPTED FOR IMPLEMENTATION and v1 numeric policy APPROVED FOR MIGRATION 069 (`DECISION_LOG.md` DEC-009: 4 policy rows — `frl_wlt1_{read_list,read_item,mutate_register,mutate_poc}` — deliberately no `AUTH_FAILURE`/per-IP row), but neither migration 068 nor 069 has been written and the platform's `/foundation/rate-limit/check` remains an allow-by-default stub; no code written for it. See FND-FIND-001 for the related HIGH pre-authentication-abuse finding, NOT solved by DEC-009's authenticated-only policy and NOT closed by this approval. | Implement and independently accept the shared rate-limit engine (BLOCKER-2) per the approved architecture and DEC-009 numeric policy, then implement the WLT-01 public-surface contract. FND-FIND-001 must also resolve before internet exposure. | None set | WLT-01 Client-Facing Public `/wlt1/*` Surface architecture review; IAM-01 Session Introspection independent Opus acceptance; Shared Rate-Limit Engine architecture + numeric policy governance review (`DECISION_LOG.md` DEC-009) | OPEN |
