...
v1.1 resolved:
1. Execution-time destination decision revalidation.
2. Revocation epoch, AML revocation epoch, whitelist and limits version binding.
3. Per-destination and per-client value, velocity and concentration controls.
4. New-destination first-use lower limit / step-up.
5. Inbound deposit-source screening and quarantine.
6. Deposit mis-attribution guard.
7. Required unhosted wallet proof-of-control.
8. Wallet own-name / KYC beneficiary binding.
9. Chain-specific address canonicalisation and name-service-to-raw resolution.
10. Address poisoning / scam / lookalike protection.
11. Supported-chain/provider coverage matrix.
12. AML-01 outcome revocation subscription.
13. Cooling-off cancellation on new risk.
