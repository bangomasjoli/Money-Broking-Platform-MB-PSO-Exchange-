...
| `clt1.cdd_outcome.read` | Read outcome aggregate |
| `clt1.cdd_outcome.receive` | Receive downstream outcome |
| `clt1.authorised_party.manage` | Manage authorised parties |
| `clt1.monitoring_feedback.receive` | Receive ongoing monitoring feedback |
| `clt1.related_party.read` | Read related-party graph |
| `clt1.data_protection_request.manage` | Manage data protection request |
...
## 7. CDD Outcome Approval Rules

Final onboarding approval requires:

```txt
kyc_kyb_outcome = pass
aml_sanctions_outcome = clear
pep_adverse_media_outcome = clear or approved_review
risk_rating_outcome = rated_and_acceptable
authorised_party_screening = pass_or_clear_for_required_parties
```
...
sanctions_hit
failed_cdd
risk_rejected_or_prohibited
prohibited_pep_adverse_media_result
```

## 8. IAM-02 Client Dual Authorisation Link

CLT-01 mandate setup must create or reference IAM-02 WF-IAM02-10 client-side dual authorisation policy.
