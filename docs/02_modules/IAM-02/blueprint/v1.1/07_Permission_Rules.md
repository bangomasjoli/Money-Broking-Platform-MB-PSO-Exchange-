...
2A. Config-sealed/CFC-01 licence lock deny cannot be overridden by user override, delegation, temporary permission, or break-glass.
...
## 8. Protected Action Registry Rules

1. Every sensitive action must be registered.
2. Unregistered sensitive action fails closed.
3. Registry entry must include permission code, approval policy, step-up requirement, licence-lock status, owner module, and test references.
4. Protected-action registry changes require approval.
5. Orphan sensitive actions block deployment.

---

## 9. Approval Binding Rules

1. Approval request must store canonical payload hash.
2. Execution must verify current payload hash.
3. Payload mismatch is Critical.
4. Decision token must bind actor, session, action, entity, payload hash, cache version, approval, and step-up.
5. Stale decision token fails closed.

---

## 10. SoD Risk Acceptance Rules

1. Critical SoD conflicts are block-only.
2. Risk acceptance requires Compliance + Management dual non-self approval.
3. Risk acceptance requires step-up.
4. Risk acceptance requires expiry and re-attestation.
5. SoD matrix editor cannot also assign roles or permissions.

---

## 11. Break-Glass Ceiling Rules

1. Break-glass permissions must be on explicit whitelist.
2. Break-glass cannot grant IAM-02 admin permissions.
3. Break-glass cannot grant SoD management.
4. Break-glass cannot grant licence-control permissions.
5. Break-glass cannot grant Exchange/prohibited permissions.
6. Break-glass cannot create durable roles/permission assignments.
7. Break-glass duration and concurrent grants are capped.
