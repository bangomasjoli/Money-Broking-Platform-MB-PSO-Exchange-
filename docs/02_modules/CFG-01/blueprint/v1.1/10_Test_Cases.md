...
## 10. Config Integrity Tests

| Test ID | Scenario | Expected Result | Priority |
|---|---|---|---|
| CFG1-TC-061 | Prohibited registry hash matches Doc00 baseline | Verified | Critical |
| CFG1-TC-062 | Licence profile row changed out-of-band | Fail closed + Critical alert | Critical |
| CFG1-TC-063 | Prohibited feature status flipped directly in DB | Fail closed + Critical alert | Critical |
| CFG1-TC-064 | Decision-time integrity check fails | Deny | Critical |
| CFG1-TC-065 | Scheduled reconciliation passes but decision-time seal fails | Deny | Critical |

---

## 11. Exchange Activation Ceremony Tests

| Test ID | Scenario | Expected Result | Priority |
|---|---|---|---|
| CFG1-TC-066 | Exchange activation without LFSA evidence verification | Blocked | Critical |
| CFG1-TC-067 | LFSA evidence is free-text/unverified | Blocked | Critical |
| CFG1-TC-068 | Exchange activation without Board sign-off | Blocked | Critical |
| CFG1-TC-069 | Prohibited registry manager approves activation | Blocked by meta-SoD | Critical |
| CFG1-TC-070 | Cooldown/time-lock not completed | Production activation blocked | Critical |
| CFG1-TC-071 | Per-feature activation missing | Feature remains locked | Critical |
| CFG1-TC-072 | Deployment gate fails during Exchange ceremony | Activation blocked | Critical |
| CFG1-TC-073 | Exchange activation completes full ceremony | Activated and re-sealed | Critical |

---

## 12. Feature Gate Coverage Tests

| Test ID | Scenario | Expected Result | Priority |
|---|---|---|---|
| CFG1-TC-074 | IAM-02 protected action requires CFG token | Enforced | Critical |
| CFG1-TC-075 | Feature-gated action calls IAM-02 without CFG token | Denied | Critical |
| CFG1-TC-076 | Downstream action never invokes CFG-01 | Orphan gate detected | Critical |
| CFG1-TC-077 | Feature gate mapping missing | Deployment blocked | Critical |

---

## 13. Asymmetric Audit Outage Tests

| Test ID | Scenario | Expected Result | Priority |
|---|---|---|---|
| CFG1-TC-078 | Locked feature during SEC-01 outage | Denied | Critical |
| CFG1-TC-079 | Allowed licensed feature during SEC-01 outage with FND outbox enqueue | Allowed + queued audit | High |
| CFG1-TC-080 | Allowed licensed feature during SEC-01 outage but outbox enqueue fails | Blocked | Critical |
| CFG1-TC-081 | Feature change during SEC-01 outage | Blocked | Critical |
| CFG1-TC-082 | Audit outage mode exceeds duration | Alert/escalate | High |

---

## 14. Environment / Token Revocation Tests

| Test ID | Scenario | Expected Result | Priority |
|---|---|---|---|
| CFG1-TC-083 | Exchange feature enabled in dev with real client data | Blocked | Critical |
| CFG1-TC-084 | Synthetic quarantined non-prod exception | Allowed only in synthetic scope | High |
| CFG1-TC-085 | Non-prod enabled locked feature promoted to prod | Deployment blocked | Critical |
| CFG1-TC-086 | Kill-switch active after token issued | Token revoked / denied | Critical |
| CFG1-TC-087 | Licence suspended after token issued | Token revoked / denied | Critical |
| CFG1-TC-088 | Prohibited registry version changes after token issued | Token stale / denied | Critical |
...
config_integrity_tests_passed = true
exchange_activation_ceremony_tests_passed = true
feature_gate_coverage_tests_passed = true
asymmetric_audit_outage_tests_passed = true
environment_token_revocation_tests_passed = true
