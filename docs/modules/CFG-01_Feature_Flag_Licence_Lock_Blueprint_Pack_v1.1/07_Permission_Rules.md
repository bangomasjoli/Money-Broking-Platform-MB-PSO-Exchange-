...
| `cfg1.exchange_activation.request` | Request Exchange activation ceremony |
| `cfg1.exchange_activation.verify_evidence` | Verify LFSA evidence |
| `cfg1.exchange_activation.approve` | Approve Exchange activation step |
| `cfg1.config_integrity.verify` | Verify config integrity |
| `cfg1.config_integrity.seal` | Seal config baseline |
...
cfg1.bypass_config_integrity
cfg1.exchange_activation_without_ceremony
cfg1.prohibited_registry_unsigned_change
cfg1.feature_gate_without_cfg_token
```
...
11. Exchange activation ceremony step.
12. Config integrity seal creation.
13. Prohibited registry removal.
...
11. Prohibited registry manager cannot approve licence activation.
12. Licence activation approver cannot approve deployment gate override.
13. Config seal controller cannot be sole prohibited registry editor.
14. Exchange activation requires Board, Compliance, Management, and deployment gate sequencing.
...
9. Config integrity mismatch deny.
10. Stale version deny/revalidate.
11. Explicit feature allow.
12. Default deny.
...
## 8. Decision Token Binding Rules

Feature decision token must bind to:

1. feature code.
2. action.
3. resource.
4. client ID / client class.
5. environment.
6. feature config version.
7. licence profile version.
8. prohibited registry version.
9. prohibited registry integrity hash.
10. IAM-02 decision context where applicable.
11. expiry.

---

## 9. All-Environment Lock Rules

1. Prohibited/Exchange-locked features are locked in production, UAT, staging, and development.
2. Non-prod exception is synthetic-data-only and quarantined.
3. Non-prod exception cannot use real client, PSO, payment, wallet, settlement, or production-like data.
4. Production deployment gate blocks any locked feature in enabled state from any source environment.
