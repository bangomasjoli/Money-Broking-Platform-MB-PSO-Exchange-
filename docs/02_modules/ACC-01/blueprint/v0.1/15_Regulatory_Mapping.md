...
| Doc 00 §10.2A (retail lock, as stated in Doc 00) | Retail onboarding disabled by default | Class check at creation (ACC-REQ-013) — defence in depth; CLT-01 owns classification |
...
| System Rules `FRZ-RULE-001/002` | Freeze/suspension triggers and scopes | Restriction `source_type` and scope vocabularies (file 06 §2.2, §3); `login_block` excluded as principal-level |
| System Rules `OFF-RULE-001` (items 1–8) | Closure cannot complete with balance, open trade/settlement/withdrawal, recon break, AML/STR restriction, unverified return destination, unapplied retention | Items 1–5 via LED-01 attestation; 6–8 remain client-level (CLT-01/compliance); ACC-01 also refuses closure under an active authority restriction |
| Workflow Map WF-26, WF-27, §33A.3 step 4 | Freeze workflow states; closure; merchant account/subaccount created by ACC-01 | Files 02, 06 |
...
The repository states a ≥ 6-year retention for order/execution records (`DEC-012` clause 1 rule 7; Module Index `OMS-01`) and defines client retention class as *to be defined* (CLT-01 `retention_period_by_class = to_be_defined`). **No retention period for account-structure records is stated anywhere in the repository**, so none is asserted; account and history rows are never deleted, which is safe under any period, and the retention class is OQ-10.
