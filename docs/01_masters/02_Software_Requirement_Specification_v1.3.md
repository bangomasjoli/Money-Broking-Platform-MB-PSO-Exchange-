...
> ## v1.3 — RE-BASELINED ON APPROVED DOC 00 v1.5, CHARTER v1.5 AND MODULE INDEX v1.4
>
> **Base documents:** `00_Licence_Scope_And_Feature_Lock_v1.5.md`, `01_Project_Charter_v1.5.md`,
> `03_Master_Module_Index_v1.4.md` (all APPROVED), `DEC-011`, `DEC-012`, **`DEC-013`**, `STR-03`.
>
> **v1.2's base documents were `00_v1.3`, `01_v1.3` and `03_v1.2` — three versions stale.**
> v1.3 re-bases the SRS on the current approved masters and on the **build-unlocked /
> production-gated capability model**.
>
> **The requirement-state rule this version introduces (§2A).** Every requirement that touches a
> regulated capability distinguishes three things that v1.2 collapsed into one:
>
> | | |
> |---|---|
> | **`IMPLEMENTED_CAPABILITY`** | whether the software exists, is specified and is tested |
> | **`ENVIRONMENT_AVAILABILITY`** | whether it is available in DEVELOPMENT / TEST / UAT / DEMO / PRODUCTION |
> | **`PRODUCTION_REGULATORY_ACTIVATION`** | whether AIX may operate it live |
>
> **These are not one boolean.** A requirement may be fully implemented, available in four
> environments, and closed in production — that is the normal state for AIX Exchange, AIX RWA
> securities capabilities and AIX Pay beyond the §5.2 baseline.
>
> **What is added.** §2A requirement-state model; §2B environment model; new requirement families
> **`SPT-SRS-*`** (AIX Spot order and execution), **`PAY-SRS-*`** (AIX Pay), **`RWA-SRS-*`**
> (AIX RWA lifecycle), **`EXG-SRS-*`** (AIX Exchange securities market); `AST-SRS-001` rewritten
> for the full instrument registry; §19 replaced.
>
> **What does not change.** Every requirement in v1.2 is retained. No control is removed, and no
> control is relaxed in a non-production environment (§7 constraint 26).
>
> **What remains permanently prohibited, in every environment including local development:**
> internal client-to-client matching, an AIX-operated order book for MB Spot, crossing or netting
> client orders, principal dealing, proprietary market making and AIX principal liquidity (§19.1).
> **The AIX Exchange matching engine (`EXG-SRS-*`) is a different domain and grants AIX Spot
> nothing.**
>
> **This SRS claims no regulatory approval AIX has not received**, and activates nothing.

## Document Control

| Item | Details |
|---|---|
| Document name | 02_Software_Requirement_Specification_v1.3.md |
...
| Version | v1.3 |
| Status | **APPROVED** — re-baselined on Doc 00 v1.5, Charter v1.5, Module Index v1.4 and `DEC-013` |
...
10. **Permanently prohibited capabilities, and production-gated requirements** (§19). *(v1.3: replaces "future-locked Exchange exclusions", which conflated a permanent prohibition with a production gate.)*
11. Testing and acceptance planning.
12. Claude Code implementation boundaries.
13. **The requirement-state model** — implemented capability, environment availability, production regulatory activation (§2A).
...
**Build / activation baseline (`DEC-013`, new in v1.3):**
...
AIX Spot full trading capability = build unlocked, production per Doc 00 §20.1
Securities in MB Spot            = prohibited in every environment
Derivatives/margin/lending/
  staking/yield                  = out of scope, no build
...
```txt
This SRS uses 03_Master_Module_Index_v1.4.md (37 modules) as the accepted
module index base. All later module blueprints must trace back to 00, 01,
02 and 03. (v1.3 correction: v1.2 cited the module index v1.2.)
```

---

## 2A. Requirement State Model

**New in v1.3. `DEC-013` clause 2; Doc 00 §1.D.**

**Every requirement in this SRS that touches a regulated capability carries three independent
states. They must never be implemented, configured, tested or documented as one boolean.**

| State | Values | Meaning for a requirement |
|---|---|---|
| **`IMPLEMENTED_CAPABILITY`** | `NOT_SPECIFIED` / `SPECIFIED` / `IMPLEMENTED` / `TESTED` | Does the software required by this requirement exist and work? |
| **`ENVIRONMENT_AVAILABILITY`** | per environment: `ENABLED` / `DISABLED` / `NOT_APPLICABLE` | May this capability be exercised in DEVELOPMENT, TEST, UAT, DEMO, PRODUCTION? |
| **`PRODUCTION_REGULATORY_ACTIVATION`** | `DISABLED_PENDING_REGULATORY_ACTIVATION` / `DISABLED_PENDING_GOVERNANCE` / `DISABLED_BY_POLICY` / `PROHIBITED_PERMANENT` / `ACTIVE` | May AIX operate this live? |

A fourth state, **`PRODUCT_ASSET_ELIGIBILITY_STATE`**, is evaluated per product × asset/instrument
× client × jurisdiction × counterparty and is owned by `AST-01` and `CFG-01` (Doc 00 §1.D).

**Binding requirements on every requirement in this document:**

| ID | Requirement |
|---|---|
| **STATE-SRS-001** | The system shall represent implemented capability, environment availability and production regulatory activation as **separate, independently readable states**. A single field named `enabled` is prohibited as the sole representation; where one exists it means production activation only |
| **STATE-SRS-002** | The system shall treat an **unknown or unreadable** value in any state as **DENY** |
| **STATE-SRS-003** | The system shall **never** infer production activation from implementation status, test results, environment availability, UI configuration or a permission grant |
| **STATE-SRS-004** | The system shall require an **affirmative, governed, maker-checker-approved act** to set any capability's production activation to `ACTIVE`. Promotion of non-production state is prohibited |
| **STATE-SRS-005** | The system shall record **who, when, on what basis and under which approval** every production activation change was made, as `SEC-01` audit evidence |
| **STATE-SRS-006** | The system shall enforce that capabilities marked `PROHIBITED_PERMANENT` are **unreachable in every environment**, with no configuration, flag, permission, domain value or approval able to reach them (§19.1) |
| **STATE-SRS-007** | Requirements whose production activation is gated shall still be **fully implemented and tested**, including their evidence, audit and reporting obligations, so that activation is a configuration act and not a build project |

---

## 2B. Environment Model

**New in v1.3. `DEC-013` clause 2; Doc 00 §1.E; Charter §26.1.**
...
| **DEMO** | Controlled demonstration to named audiences | Mock / synthetic / non-live only |
| **PRODUCTION** | Live regulated operation | Doc 00 §21 production activation gate |

| ID | Requirement |
|---|---|
| **ENV-SRS-001** | The system shall support the five environments above as a first-class capability-control dimension |
| **ENV-SRS-002** | The system shall treat an **unknown or unrecognised environment as PRODUCTION** and fail closed |
| **ENV-SRS-003** | The system shall prohibit **real client PII, live counterparty/LP credentials and live custodian credentials** in DEVELOPMENT, TEST, UAT and DEMO |
| **ENV-SRS-004** | The system shall ensure a **controlled demo never** routes a real client order to a real venue, moves real client money, settles a real transaction or issues a real instrument |
| **ENV-SRS-005** | The system shall prevent any non-production capability state, feature state or configuration from **propagating** to PRODUCTION |
| **ENV-SRS-006** | The system shall apply **every control identically in all five environments** — audit, permission, KYC/KYB, AML, Travel Rule, ledger and balance integrity, client approval, LP settlement approval and break-glass logging. No control is relaxed for non-production convenience |
| **ENV-SRS-007** | The system shall treat a **pre-production mirror of production** as inheriting PRODUCTION's activation gate, not UAT's, and shall fail closed where an environment identifier does not map to one of the five (`MIG-005`) |
| **ENV-SRS-008** | The system shall record the environment in every capability decision and audit record |
...
7. OTC/RFQ broking.
8. **AIX Spot** — client order management, pre-trade controls, market data, external execution routing (v1.3: renamed from "MB Spot Broking Terminal", Doc 00 §2A).
9. External LP-backed agency execution, provider-neutral and multi-venue capable.
...
18. **AIX Pay** — the full merchant and payment platform (§14B).
19. **AIX RWA** — the full asset-tokenisation lifecycle (§14C).
20. **AIX Exchange** — the securities / financial-instrument / security-token market capability (§14D).

**Items 18–20 are built and tested; their production activation is separately gated** (§2A,
§19.2).

**It does not, and must never, support an AIX-operated digital-currency exchange**: no internal
client-to-client matching, no AIX order book for MB Spot, no market making, no principal
dealing (§19.1). That prohibition holds in every environment and is not affected by AIX Exchange,
which is a securities-domain capability in a different regulatory route.
...
10. Use the AIX Spot trading interface if approved.
11. Request deposit.
...
**Reclassified in v1.3.** v1.2's list mixed permanent prohibitions with production gates and
said "in MVP", which reads as a delivery-phase exclusion rather than a boundary. Constraints
1–6 are **permanent and environment-independent**; 7–13 are scope or activation constraints.
**Nothing is removed.**

1. **No AIX order book for MB Spot — permanently, in every environment including local development.**
2. **No matching engine for MB Spot — permanently, in every environment.** *(The AIX Exchange matching engine, §14D, is the securities domain and is not reachable from any MB-domain module.)*
3. **No client-to-client matching, crossing or netting — permanently, in every environment.**
4. **No market making — permanently.**
5. **No principal dealing or AIX principal liquidity — permanently.**
6. **No proprietary trading — permanently.**
7. No derivatives, futures or perpetuals — **out of scope, no build**.
8. No margin or leveraged trading — **out of scope, no build**.
9. **No securities token in production** until the applicable regulatory activation condition is satisfied. **Development, automated testing, UAT and controlled demo against synthetic instruments are permitted** (§14C, §14D). *(v1.3 change: v1.2's "unless separately approved" gave no build dimension.)*
9a. **No asset classified `SECURITY / SECURITY TOKEN` may enter AIX Spot or AIX OTC**, in any environment (Doc 00 §12A).
10. No privacy coins.
...
23. **No lending, staking, yield or earn product — out of scope, no build.**
24. **No production activation inferred from implementation status, test results, environment availability or a permission grant** (`STATE-SRS-003`).
25. **No non-production capability state promoted to production** (`ENV-SRS-005`).
26. **No control relaxed in a non-production environment** (`ENV-SRS-006`).
27. **No synthetic or test instrument classification valid in PRODUCTION**, and none promotable into a production classification (`AST-SRS-001A`).
28. **No MB-domain module may call an Exchange-domain module**, and no Exchange-domain module may execute an AIX Spot or AIX OTC client order (§14D, Module Index §19 rule 5A).
...
### AST-SRS-001 Asset and Instrument Registry

**Rewritten in v1.3** for the full product ecosystem (Doc 00 §12A; Module Index §9; `AST-01`).
v1.2's eight fields described a digital-currency asset master and cannot carry a security token,
a tokenised fund or a tokenised debt instrument.

The system shall maintain a single platform-wide asset and instrument registry.

**Supported asset / instrument types:**

```txt
fiat · digital currency · stablecoin · security · security token
RWA token · tokenised debt · tokenised fund · tokenised commodity
...
| Group | Fields |
|---|---|
| **Identity** | Instrument code; name; asset class; asset type; decimal precision |
| **Legal / regulatory** | **Legal and regulatory classification** (the securities-features gate); classifier of record; evidence reference; classification status |
| **Origin** | Issuer; underlying asset; jurisdiction |
| **Technical** | Network; contract address; custody capability |
| **Product eligibility** | **Spot eligibility; OTC eligibility; Pay eligibility; RWA eligibility; Exchange eligibility** — each **derived from** classification, never set independently |
| **Operational status** | Deposit status; withdrawal status; **listing status**; transfer restrictions |
| **Access** | Client eligibility; jurisdiction eligibility |
| **Activation** | **Production activation status** (§2A) |
| **Controls** | Risk classification; admissibility status; compliance approval status |

**Requirements:**

1. Classification shall **precede** every product eligibility determination.
2. Per-product eligibility shall be **derived from** classification and shall not be settable independently of it.
3. **Unresolved classification is the default state on instrument creation** and shall **fail closed for production activation**.
4. An instrument classified `SECURITY / SECURITY TOKEN` shall **never** be eligible for AIX Spot or AIX OTC, in any environment.
5. An instrument's Exchange eligibility shall be reachable only through a resolved `SECURITY / SECURITY TOKEN` classification and the `EXM-01` admission path (§14D).
6. Classification changes shall require maker-checker approval and shall emit audit evidence.

### AST-SRS-001A Synthetic and Test Instruments

**New in v1.3.** Development, automated testing, UAT and controlled demo of AIX RWA and AIX
Exchange require instruments to exist before any real instrument is classified (`MIG-010`).

1. The registry shall support a **`SYNTHETIC` / `TEST_INSTRUMENT`** classification.
2. A synthetic classification shall be **valid only in DEVELOPMENT, TEST, UAT and DEMO** and shall **fail closed in PRODUCTION**.
3. A synthetic instrument shall **never** be promotable into a production classification; a real instrument is classified afresh, on evidence.
4. A synthetic classification shall never be cited as evidence about a real asset.
5. Synthetic instruments shall be visually and structurally distinguishable in every surface that displays them.
...
### PRD-SRS-005 AIX Spot Trading Interface

> **v1.3: retitled from "MB Spot Broking Terminal"** (Doc 00 §2A terminology). The requirements
> below are retained from v1.2 and describe the **quote-and-confirm** interaction. They are
> **extended, not replaced**, by §14A's order-and-execution requirements (`SPT-SRS-*`), which
> carry the Model A client order store, pre-trade controls, routing and order types. Where the
> two differ, §14A is normative.

The system shall provide the AIX Spot trading interface.
...
## 14A. Functional Requirements — AIX Spot Order and Execution

**New in v1.3.** Doc 00 §7, §11B, §20.1; `DEC-012` Model A/B; modules `OMS-01`, `MKD-01`,
`LQD-01`, `EXE-01`, `TRD-01`, `SUR-01`.

**All requirements below are `IMPLEMENTED_CAPABILITY` targets, available in DEVELOPMENT / TEST /
UAT / DEMO against mock venues.** Production activation follows Doc 00 §20.1 and the per-venue
gates in Doc 00 §7A.

| ID | Requirement | Production activation |
|---|---|---|
| **SPT-SRS-001** | The system shall provide an institutional trading interface and API for AIX Spot | Per Doc 00 §20.1 |
| **SPT-SRS-002** | The system shall ingest, normalise and display external market data, charts and **external / aggregated market depth**, labelled as externally sourced and never as an AIX-operated order book | Per Doc 00 §20.1 |
| **SPT-SRS-003** | The system shall maintain a **client order store** recording each client's own instructions and their full lifecycle, including **unexecuted and cancellable** orders (LFSA-DMB-2025 ¶5.5, ¶6.4(i)) | Per Doc 00 §20.1 |
| **SPT-SRS-004** | The client order store shall **never** hold mutually executable AIX client orders. No two client orders may be matched, crossed, netted or internalised against each other | **`PROHIBITED_PERMANENT`** — every environment |
| **SPT-SRS-005** | The system shall present open orders, order history and trade history, with **≥ 6-year retention** (¶5.12) | Per Doc 00 §20.1 |
| **SPT-SRS-006** | The system shall support **Market**, **Limit** and **Cancel** order types | **DISABLED** pending product-rule review (Doc 00 §11B) |
| **SPT-SRS-007** | The system shall support **Stop, Stop-Limit, IOC, FOK and GTC** order types **implemented behind product/configuration gates** | **DISABLED** pending `R3-Q3` |
| **SPT-SRS-008** | The system shall support **partial fills** and **split fills** across venues, with fill conservation enforced | Per Doc 00 §20.1 |
| **SPT-SRS-009** | The system shall apply **automated pre-trade controls** before execution: order price and quantity controls, suspicious-order detection and blocking, sufficient-funds check (¶5.2, ¶6.4) | Per Doc 00 §20.1 |
| **SPT-SRS-010** | The system shall route orders for execution to **approved external counterparties / venues** only, through a **provider-neutral** liquidity abstraction and venue adapter layer | Venue gates, Doc 00 §7A |
| **SPT-SRS-011** | The system shall support **multi-LP / multi-venue smart routing** (Model B), evaluating price, available depth, fees, expected slippage, venue health, counterparty exposure, supported assets, settlement capability, regulatory eligibility and operational availability. **"Lowest price always wins" is prohibited** | **DISABLED per venue** — §7A incl. seven-day notification |
| **SPT-SRS-012** | The system shall make **every routing decision reconstructable and disclosable** (¶9.7(i), ¶9.5) | Per Doc 00 §20.1 |
| **SPT-SRS-013** | The system shall provide a **trading API** for institutional clients, through `API-01` | Per Doc 00 §20.1 |
| **SPT-SRS-014** | The system shall provide **market surveillance**: order and trade surveillance, suspicious-pattern detection, trading risk limits and post-trade monitoring. **This is a required control**, not an optional capability (¶6.4(i)) | Per Doc 00 §20.1 |
| **SPT-SRS-015** | The system shall provide Spot reporting and post-trade transparency (¶5.9) | Per Doc 00 §20.1 |
| **SPT-SRS-016** | The system shall support **execution against displayed market depth** behind a product gate | **DISABLED** pending `R3-Q2b` |
| **SPT-SRS-017** | No AIX Spot component shall call, reach or depend on an Exchange-domain module (§14D) | **`PROHIBITED_PERMANENT`** — every environment |
| **SPT-SRS-018** | No AIX Spot component shall provide AIX principal liquidity, quote as principal, make a market, or hold AIX inventory | **`PROHIBITED_PERMANENT`** — every environment |

---

## 14B. Functional Requirements — AIX Pay

**New in v1.3.** Doc 00 §12D, §20.3; Charter §12A.4; module `PAY-01`.

**Build-unlocked.** All requirements below are implementation targets available in DEVELOPMENT /
TEST / UAT / DEMO. **Production activation of anything beyond the Doc 00 §5.2 baseline is
`DISABLED_PENDING_REGULATORY_ACTIVATION`** pending PSO-scope verification (`R5-Q1`) and fee-model
basis (`R5-Q2`).

| ID | Requirement |
|---|---|
| **PAY-SRS-001** | Merchant onboarding, including merchant due diligence through `KYC-01` and `AML-01` |
| **PAY-SRS-002** | Merchant organisation structure, merchant users and merchant roles, scoped through `IAM-02` and `ACC-01`; **no second identity or membership system** |
| **PAY-SRS-003** | **Payment intent** lifecycle |
| **PAY-SRS-004** | **Hosted checkout** |
| **PAY-SRS-005** | **API checkout** |
| **PAY-SRS-006** | **QR payment** |
| **PAY-SRS-007** | **Invoices** |
| **PAY-SRS-008** | Payment **status lifecycle** with an explicit state machine (§22) |
| **PAY-SRS-009** | **Digital-asset payments**, restricted to instruments eligible for Pay in `AST-01` |
| **PAY-SRS-010** | **Conversion orchestration** where applicable, through the approved external execution path — never internal or synthetic pricing |
| **PAY-SRS-011** | **Settlement** to the merchant |
| **PAY-SRS-012** | **Payout**, restricted to **whitelisted settlement destinations** |
| **PAY-SRS-013** | **Refund**, with ledger reversal and fee reversal |
| **PAY-SRS-014** | **Fee engine**, through `FEE-01`, with disclosed-fee evidence |
| **PAY-SRS-015** | Merchant **API keys**, issued and revoked through `API-01` |
| **PAY-SRS-016** | **Webhooks**, signed and replay-resistant |
| **PAY-SRS-017** | **Reconciliation** of merchant settlement, through `REC-01` |
| **PAY-SRS-018** | **Merchant reporting**, through `RPT-01` |
| **PAY-SRS-019** | **Maker-checker** on merchant settlement destination changes, payout release and fee-schedule changes |
| **PAY-SRS-020** | **Risk controls** — limits, velocity, exposure |
| **PAY-SRS-021** | A **public hosted checkout surface** is a pre-authentication surface and shall not be internet-exposed until its own perimeter analysis is complete and outstanding pre-authentication findings are closed |
| **PAY-SRS-022** | `PAY-01` shall consume `LED-01`, `FEE-01`, `REC-01`, `CLT-01`, `KYC-01`, `AML-01` and `SEC-01`; it shall **not** re-implement ledger, compliance, settlement, reconciliation or audit |

---

## 14C. Functional Requirements — AIX RWA

**New in v1.3.** Doc 00 §12B, §20.4; Charter §12A.5; modules `RWA-01`…`RWA-04`, `AST-01`.

**Build-unlocked against synthetic assets** (`AST-SRS-001A`). **Production activation is
`DISABLED_PENDING_REGULATORY_ACTIVATION`** pending `R4-Q1`…`R4-Q7`; the security-token route
additionally pending `R4-Q2` and `R1-Q1b`.

| ID | Requirement |
|---|---|
| **RWA-SRS-001** | **Issuer onboarding** and issuer profile, through `CLT-01` |
| **RWA-SRS-002** | **Issuer KYB, UBO and due diligence**, through `KYC-01` and `AML-01` — never re-implemented |
| **RWA-SRS-003** | **Asset onboarding** |
| **RWA-SRS-004** | **Asset evidence and verification**, with evidence retention |
| **RWA-SRS-005** | **Regulatory / legal classification** through `AST-01` — **this precedes every downstream stage** |
| **RWA-SRS-006** | **Structuring** within the classified route |
| **RWA-SRS-007** | **Token configuration**, including supply model and transfer-restriction configuration |
| **RWA-SRS-008** | **Smart-contract orchestration**, constrained by the third-party custody requirement (Doc 00 §8.2, `R4-Q5`). Non-production shall use testnet or mock deployment only |
| **RWA-SRS-009** | **Offering** setup and terms |
| **RWA-SRS-010** | **Subscription** intake |
| **RWA-SRS-011** | **Investor eligibility** determination, reusing client classification and compliance status |
| **RWA-SRS-012** | **Allocation** of subscribed amounts |
| **RWA-SRS-013** | **Issuance** and initial holder recording |
| **RWA-SRS-014** | **Token / holder registry** over the instrument's life (`R4-Q4` unresolved — production gated) |
| **RWA-SRS-015** | **Transfer controls** with enforced transfer restrictions |
| **RWA-SRS-016** | **Asset servicing** |
| **RWA-SRS-017** | **Corporate actions** |
| **RWA-SRS-018** | **Distributions** to holders |
| **RWA-SRS-019** | **Reporting** — issuer, holder and regulatory, through `RPT-01` |
| **RWA-SRS-020** | **Redemption** and retirement |
| **RWA-SRS-021** | **Secondary-market eligibility**, derived from classification in `AST-01` and never set independently |
| **RWA-SRS-022** | Where classification is `SECURITY / SECURITY TOKEN` and secondary-market eligibility is granted, the instrument routes to **`EXM-01` Exchange admission** (§14D) — **not** to AIX Spot or AIX OTC |
| **RWA-SRS-023** | **Unresolved classification shall prevent production activation of that instrument** and shall **not** prevent development, synthetic testing, workflow development, UAT or controlled demo of the platform |
| **RWA-SRS-024** | RWA modules shall consume identity, KYB/UBO, AML, the account hierarchy, `AST-01`, `LED-01`, settlement, reconciliation and audit; they shall **not** re-implement them |

---

## 14D. Functional Requirements — AIX Exchange (Securities Market)

**New in v1.3.** Doc 00 §12E, §20.5; Charter §12A.6; Module Index §16A; modules `EXM-01`,
`EXO-01`, `EXC-01`, `EXP-01`.

**Domain: securities / financial instruments / security tokens. Not digital-currency Spot.**

**Build-unlocked** (`DEC-013` clause 7), available in DEVELOPMENT / TEST / UAT / DEMO against
**synthetic instruments with non-live execution**. **Production activation is
`DISABLED_PENDING_REGULATORY_ACTIVATION`** pending `R1-Q1b` and Doc 00 §21.

| ID | Requirement | Module |
|---|---|---|
| **EXG-SRS-001** | **Instrument admission** to the Exchange, on a resolved `SECURITY / SECURITY TOKEN` classification | `EXM-01` |
| **EXG-SRS-002** | **Listing** and listing status lifecycle | `EXM-01` |
| **EXG-SRS-003** | **Market configuration** for securities and security tokens | `EXM-01` |
| **EXG-SRS-004** | **Trading calendar and sessions** | `EXM-01` |
| **EXG-SRS-005** | **Trading halts**, with reason, authority and audit evidence | `EXM-01` |
| **EXG-SRS-006** | **Market operations** surfaces | `EXM-01` |
| **EXG-SRS-007** | **Order entry** for Exchange participants | `EXO-01` |
| **EXG-SRS-008** | **Central order book** holding mutually executable participant orders **in the securities domain only** | `EXO-01` |
| **EXG-SRS-009** | **Matching engine** with deterministic, auditable **price/time priority** | `EXO-01` |
| **EXG-SRS-010** | **Execution** generation from matches, with full execution evidence | `EXO-01` |
| **EXG-SRS-011** | **Exchange market data and market depth** | `EXO-01` |
| **EXG-SRS-012** | **Exchange order history and trade history**, with ≥ 6-year retention | `EXO-01` |
| **EXG-SRS-013** | **Investor / participant eligibility** for the Exchange | `EXP-01` |
| **EXG-SRS-014** | **Market permissions** | `EXP-01` |
| **EXG-SRS-015** | **Transfer restrictions** enforced at the market boundary | `EXP-01` |
| **EXG-SRS-016** | **Participant admission** | `EXP-01` |
| **EXG-SRS-017** | **Clearing / settlement interface** | `EXC-01` |
| **EXG-SRS-018** | **Corporate-action interaction** | `EXC-01` |
| **EXG-SRS-019** | **Holder-registry and transfer-control handoff** to `RWA-04` and `AST-01` | `EXC-01` |
| **EXG-SRS-020** | **Surveillance** of Exchange orders and trades — `SUR-01` **extends**; no second surveillance module | `SUR-01` |
| **EXG-SRS-021** | **Exchange reporting and audit evidence** — `RPT-01` and `SEC-01` extend | `RPT-01`, `SEC-01` |
| **EXG-SRS-022** | **Exchange APIs** — `API-01` extends; perimeter analysis required before any exposure | `API-01` |

### EXG-SRS-100 The Exchange boundary — permanent, environment-independent
...
|---|---|---|
| **EXG-SRS-101** | No Money-Broking-domain module — `OMS-01`, `MKD-01`, `LQD-01`, `EXE-01`, `TRD-01`, `SUR-01` acting in its MB capacity — shall call `EXO-01`, `EXM-01`, `EXC-01` or `EXP-01` | **`PROHIBITED_PERMANENT`** |
| **EXG-SRS-102** | No Exchange-domain module shall execute an AIX Spot or AIX OTC client order | **`PROHIBITED_PERMANENT`** |
| **EXG-SRS-103** | The boundary shall hold identically in DEVELOPMENT, TEST, UAT, DEMO and PRODUCTION, under any configuration, capability state or approval outcome | **`PROHIBITED_PERMANENT`** |
| **EXG-SRS-104** | `EXO-01` shall carry this boundary as a **module-level invariant with its own automated test evidence**, not as documentation | Required |
| **EXG-SRS-105** | Execution code shall **not** be shared between `EXE-01` and `EXO-01`. Shared platform infrastructure is consumed from its owning module as normal | Required |
| **EXG-SRS-106** | Exchange capabilities shall use the reserved **`securities_market.*`** namespace. The `exchange.*` namespace is frozen as the MB-boundary prohibition namespace and shall not be reused (`MIG-007`, `DEC-013` clause 10) | Required |
...
## 19. Permanently Prohibited Capabilities and Production-Gated Requirements

**Replaced in v1.3.** v1.2's "Future-Locked Exchange Requirements" listed twelve items under one
heading that mixed three different things: capabilities that must never exist, order types
already reclassified by Doc 00 §11B, and a required control (**market surveillance**). A
placeholder section also gave the securities Exchange **no requirements at all**, which made it
unbuildable (`STR-03` L-42). **Nothing is removed from the platform's prohibitions.**

### 19.1 Permanently prohibited — every environment, no activation path

| # | Capability | State |
|---|---|---|
| 1 | AIX order book **for MB Spot** | `PROHIBITED_PERMANENT` |
| 2 | Matching engine **for MB Spot** | `PROHIBITED_PERMANENT` |
| 3 | AIX-operated market depth presented as an AIX market | `PROHIBITED_PERMANENT` |
| 4 | AIX-operated public exchange trading for digital currency | `PROHIBITED_PERMANENT` |
| 5 | **Client-to-client matching, crossing, netting, internalisation** | `PROHIBITED_PERMANENT` |
| 6 | Public market API **for an AIX-operated market** | `PROHIBITED_PERMANENT` |
| 7 | Market maker engine | `PROHIBITED_PERMANENT` |
| 8 | Maker/taker fee engine | `PROHIBITED_PERMANENT` |
| 9 | Principal dealing, proprietary trading, AIX principal liquidity, AIX inventory | `PROHIBITED_PERMANENT` |
| 10 | Reuse of `EXO-01` by any MB-domain module | `PROHIBITED_PERMANENT` |

**Requirements:**

1. `IMPLEMENTED_CAPABILITY` shall remain **`NOT_SPECIFIED`** for every item above, permanently.
2. These shall be **unreachable in DEVELOPMENT, TEST, UAT, DEMO and PRODUCTION**. No feature flag, permission, configuration, service domain, environment value or future Exchange approval may reach them.
3. They shall not be wired into any runtime, in any environment.
4. They shall not be visible to clients.
5. They shall **never share an execution path with AIX Spot or AIX OTC**.
6. Backend feature locks shall remain in force, and the seeded prohibited-feature registry shall continue to deny them.
7. **There is no activation procedure.** Lifting any of items 1–10 would require a different licence basis and a new licence-scope revision, not a feature activation (Doc 00 §6).

### 19.2 Built and tested, production activation gated

**These are not in §19.1. They are implementation targets now** (`DEC-013` clause 7).

| Capability | Requirements | Production activation |
|---|---|---|
| AIX Exchange securities market | `EXG-SRS-001`…`EXG-SRS-022` (§14D) | `DISABLED_PENDING_REGULATORY_ACTIVATION` — `R1-Q1b` |
| Security-token issuance and secondary market | `RWA-SRS-*`, `EXG-SRS-*`, `AST-SRS-001` | `DISABLED_PENDING_REGULATORY_ACTIVATION` — `R4-Q2`, `R1-Q1b` |
| Non-security RWA issuance and servicing | `RWA-SRS-*` (§14C) | `DISABLED_PENDING_REGULATORY_ACTIVATION` — `R4-Q6`, `R4-Q7` |
| AIX Pay beyond the Doc 00 §5.2 baseline | `PAY-SRS-*` (§14B) | `DISABLED_PENDING_REGULATORY_ACTIVATION` — `R5-Q1`, `R5-Q2` |
| Multi-venue smart routing (Model B) | `SPT-SRS-011`, `SPT-SRS-012` | `DISABLED` per venue — Doc 00 §7A |
| Order types Stop, Stop-Limit, IOC, FOK, GTC | `SPT-SRS-007` | `DISABLED` — `R3-Q3`; implementable behind product/configuration gates |
| Execution against displayed depth | `SPT-SRS-016` | `DISABLED` — `R3-Q2b` |

**Requirement `GATE-SRS-001`.** Each capability above shall be **fully implemented and tested,
including its evidence, audit, retention and reporting obligations**, so that production
activation is a governed configuration act and not a build project (`STATE-SRS-007`).

### 19.3 Corrections to v1.2's list

| v1.2 item | v1.3 treatment |
|---|---|
| 9. Resting Limit Orders | **Not a licence lock.** Doc 00 §6 reclassified order types into §11B's product-rule gate. `SPT-SRS-006`/`SPT-SRS-007` |
| 10. Stop-Limit Orders | As above |
| 11. GTC / Post-Only / IOC / FOK Orders | As above. **Post-only and maker/taker remain prohibited** as MB-boundary and revenue-model matters (§19.1 item 8) |
| 12. Public Exchange Market Surveillance | **Not locked. It is a required control** under LFSA-DMB-2025 ¶6.4(i), owned by `SUR-01`. `SPT-SRS-014`, `EXG-SRS-020` |

### 19.4 Out of product scope — no requirements, no development effort

Derivatives, perpetual futures, futures, margin trading, leveraged trading, lending, borrowing
of client assets, staking, yield / earn products, DeFi yield, privacy coins, algorithmic
stablecoins where currently prohibited, MYR trading pairs where currently prohibited, and
self-custody wallet service. **This SRS states no requirements for any of them**
(`DEC-013` clause 6; Doc 00 §20.6).
...
19. **Client order lifecycle** (`SPT-SRS-003`). *(New in v1.3.)*
20. **Instrument classification** (`AST-SRS-001`).
21. **Capability production activation** (`STATE-SRS-004`).
22. **Payment intent / payment status** (`PAY-SRS-008`).
23. **Merchant onboarding** (`PAY-SRS-001`).
24. **RWA asset lifecycle** — proposal through redemption (`RWA-SRS-*`).
25. **RWA offering and subscription** (`RWA-SRS-009`…`RWA-SRS-012`).
26. **Exchange instrument admission and listing status** (`EXG-SRS-001`, `EXG-SRS-002`).
27. **Exchange order lifecycle** (`EXG-SRS-007`…`EXG-SRS-010`).
28. **Trading halt** (`EXG-SRS-005`).
29. **Exchange participant admission** (`EXG-SRS-016`).
...
10. No direct access to prohibited or production-gated capabilities.
11. **Capability evaluation through `CFG-01`**, carrying environment, product, asset/instrument, client and counterparty dimensions. *(New in v1.3.)*
12. **Environment recorded** in the decision and the audit record (`ENV-SRS-008`).
13. **No production activation inferred** from a successful permission check (`STATE-SRS-003`).
...
30. **Capability-state tests** — that implemented capability, environment availability and production regulatory activation are independently readable and that no one implies another (`STATE-SRS-001`, `STATE-SRS-003`). *(New in v1.3.)*
31. **Per-environment capability tests** — that a capability enabled in DEVELOPMENT, TEST, UAT and DEMO is denied in PRODUCTION while its gate is closed (`ENV-SRS-005`).
32. **Fail-closed tests** — unknown environment, unknown activation state, unreadable configuration and missing classification each deny (`STATE-SRS-002`, `ENV-SRS-002`).
33. **Permanent-prohibition tests** — that each §19.1 capability is unreachable in every environment, and that no feature flag, permission, configuration or domain value reaches it.
34. **Exchange boundary tests** — that no MB-domain module can call `EXO-01`, `EXM-01`, `EXC-01` or `EXP-01`, and that no Exchange-domain module can execute a Spot or OTC client order (`EXG-SRS-101`…`EXG-SRS-104`). **Required as `EXO-01` module-level invariant evidence.**
35. **Synthetic-instrument tests** — that a synthetic classification fails closed in PRODUCTION and is not promotable (`AST-SRS-001A`).
36. **Order lifecycle tests** — recording, unexecuted orders, cancellation, partial fills, split fills, fill conservation (`SPT-SRS-003`…`SPT-SRS-008`).
37. **Routing evidence tests** — that every routing decision is reconstructable and disclosable (`SPT-SRS-012`).
38. **Securities-exclusion tests** — that no instrument classified `SECURITY / SECURITY TOKEN` can be admitted to AIX Spot or AIX OTC, in any environment (§7 constraint 9a).
39. **RWA lifecycle tests against synthetic assets**, end to end (`RWA-SRS-*`).
40. **Exchange matching tests** — deterministic price/time priority, auditable (`EXG-SRS-009`).
41. **Promotion tests** — that no non-production capability state propagates to PRODUCTION (`ENV-SRS-005`).
...
19. **Permanently prohibited capabilities verified unreachable in every environment** (§19.1). *(v1.3: replaces "Exchange-locked modules disabled", which no longer describes the control accurately.)*
19a. **Every production-gated capability verified `DISABLED` in PRODUCTION** and its gate recorded (§19.2).
19b. **Synthetic and test instruments verified to fail closed in PRODUCTION** (`AST-SRS-001A`).
19c. **Environment mapping verified**, with any unmapped environment failing closed (`ENV-SRS-007`).
...
25. **Per-environment capability availability configuration** for every production-gated capability (`MIG-004`).
26. **The pre-production mirror's environment mapping** (`MIG-005`).
27. **The `securities_market.*` capability code set** and its seeded default states (`MIG-007`).
28. **Exchange market microstructure parameters** — tick size, lot size, price bands, auction and session definitions, halt thresholds.
29. **Exchange clearing and settlement counterparties and their interfaces** (`EXG-SRS-017`).
30. **RWA custody basis for tokenised instruments** (`R4-Q5`).
31. **Pay and RWA fee models** (`R5-Q2`).
32. **The legal classifier of record and evidence standard** for the securities-features test (`R4-Q3`).
33. **Whether external controlled demo of a production-gated regulated capability is permissible** (`R6-Q1`).
...
```txt
> **HISTORICAL ARTEFACT. Retained verbatim, not rewritten.** The prompt below is the review
> prompt used for **v1.2** and refers to v1.2's product framing and base documents. It is **not**
> a live instruction and is **not** the review prompt for v1.3. A v1.3 review prompt must cover
> the requirement-state model (§2A), the environment model (§2B), the `SPT`/`PAY`/`RWA`/`EXG`
> requirement families (§14A–§14D), the Exchange boundary (`EXG-SRS-100`), and §19's separation
> of permanent prohibitions from production gates.

Review this 02_Software_Requirement_Specification_v1.2.md as a principal fintech platform architect.
