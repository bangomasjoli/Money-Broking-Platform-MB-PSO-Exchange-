...
| `foundation.scheduled_job_registered` | Scheduled job registered | Medium |
| `foundation.scheduled_job_started` | Scheduled job started | Low |
| `foundation.scheduled_job_completed` | Scheduled job completed | Low |
| `foundation.scheduled_job_failed` | Scheduled job failed | High |
| `foundation.scheduled_job_missed` | Scheduled job missed | Critical/High |
| `foundation.job_enqueued` | Background job enqueued | Low |
| `foundation.job_dead_lettered` | Job moved to dead-letter | High/Critical |
| `foundation.async_correlation_missing` | Async correlation missing | High |
| `foundation.rate_limit_throttled` | Rate limit throttled/blocked | Medium/High |
| `foundation.db_isolation_violation` | DB role/RLS violation | Critical |
| `foundation.audit_outbox_unavailable` | Required audit/outbox unavailable | Critical |
...
## 5. Scheduler / Job Audit Requirements

Scheduler and job events must include:
...
7. idempotency_key.
8. status.
9. retry/dead-letter status where applicable.

## 6. Transaction-Coupled Audit / Outbox Requirements

Sensitive and money-critical events must prove:

1. audit/outbox event is written in the same transaction as the action, or
2. guaranteed outbox pattern is used with reconciliation.

If the required audit/outbox write fails, the sensitive action must fail closed.

## 7. Async Correlation Requirements

Async audit events must preserve:

1. origin_correlation_id.
2. current correlation_id.
3. causation_id.
4. source request/event/job reference.
