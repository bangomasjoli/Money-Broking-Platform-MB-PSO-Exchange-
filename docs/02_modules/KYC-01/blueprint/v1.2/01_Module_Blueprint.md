...
| UBO look-through | Recurse to ultimate natural persons; untraceable = EDD/fail |
| Indirect ownership aggregation | Required |
| Trust/nominee role model | Required |
| Vendor reliance | Approved/accredited, records-obtainable, confidence/validity controlled |
| Vendor degraded mode | Fail closed or approved manual fallback |
| Proofing assurance | Defined by client class/risk/channel |
| Remote liveness/biometric binding | Required where policy says non-face-to-face |
| SoW/SoF | Required for EDD/high-risk/HNWI where policy says |
| KYC/AML boundary | KYC pass is identity/entity only, not screening clear |
| Evidence-store security | Encryption, IAM-02 access, hash re-verification, retention/destruction |
| Verified identity hash basis | Aligned with CLT-01 |
| Minimum EDD measures | Required |
...
v1.1 resolved:
1. UBO look-through to natural persons.
2. Vendor reliance / degraded-mode framework.
3. Identity-proofing assurance.
4. KYC/AML boundary hard contract.
5. Evidence-store security contract.
6. Identity-vs-application cross-check.
7. Verified-identity hash alignment with CLT-01.
8. Minimum EDD measures.
