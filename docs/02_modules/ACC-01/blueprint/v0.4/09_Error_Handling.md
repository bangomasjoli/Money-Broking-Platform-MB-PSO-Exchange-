...
| `ACC1_DEPENDENCY_NOT_SATISFIED` | 503 | High | **(R2, R3)** A named dependency prerequisite (`DEP-IAM-ACTOR-BINDING`, `DEP-IAM-ENTITLEMENT`, `DEP-IAM-SCOPED-CREDENTIAL`, `DEP-CLT-READ-SCOPE`, `DEP-LED-CLOSURE-CONTRACT`, `DEP-FREEZE-GOVERNANCE`, `DEP-PUBLIC-PERIMETER`; file 01 §4.5) is not evidenced **by behaviour verified from the owning service or by an authoritative governance source**. Carries the dependency id, an `evidence_failure` code (`SEAM_ABSENT`, `EVIDENCE_MISSING`, `EVIDENCE_MALFORMED`, `EVIDENCE_MISMATCH`, `SCOPE_NOT_STATED`, `GOVERNANCE_HARD_UNSATISFIED`) and the evidence `provider`; never prerequisite internals and **never an environment name**. A configuration value can never turn it off. Replaces v0.2 `ACC1_REAL_USE_NOT_PERMITTED` |
...
| `ACC1_DEFAULT_SUBACCOUNT_PROTECTED` | 409 | High | The default subaccount can **never** be closed, completed or aborted independently while its master is non-`closed`; it enters `closing` only with its master's closure and becomes `closed` only in the same transaction as its master (ACC-R3-HD-01; v0.2's "only after its master closes" and v0.3's "closes before the master seals" are both removed) |
| `ACC1_CLOSURE_FAMILY_MEMBER` | 409 | High | **(R3)** Action refused because the target is a **master-directed** family member: individual completion, individual abort, independent initiation under a master that is `closing` or later, or re-initiation of a member |
| `ACC1_CLOSURE_FAMILY_SET_CHANGED` | 409 | High | **(R3)** Master initiation (or a master seal/abort payload) does not match the live family set (a subaccount was created, closed or re-attested since the preview/approval). Nothing changed; make a new request |
| `ACC1_CLOSURE_FAMILY_INELIGIBLE` | 409 | High | **(R3)** Master seal refused: a master-directed child is not `closure_sealed` with a fresh clear attestation, or an **independent** child is not `closed` |
| `ACC1_CLOSURE_APPROVAL_STALE` | 409 | High | **(R3-F02)** Seal apply refused: the target `version` or `closure_cycle` differs from what the checker approved, the pinned readiness is no longer the latest, or the approved payload no longer matches. New request, new approval |
| `ACC1_CLOSURE_READINESS_REFUSED` | 409 | High | **(R3-F02)** A readiness insert was refused by the database: the target is not `closing` at that cycle/initiation (typically a collection that raced the seal). Not recorded; not considered |
...
| `ACC1_CLOSURE_BLOCKED` | 409 | High | Completion refused: latest attestation `blocked`/`unavailable`/stale/pre-barrier/`seal_version` mismatch, `binding_ok = false`, `committed_after_preseal_watermark > 0`, `max_resolution_version_committed ≥ closure_sealed_at_version`, `in_flight_status = unresolved`, watermark differs from the **pinned** readiness watermark, balance/open items not drained, a family member failing re-verification, changed family set hash, authority restriction active. Body lists attester and member outcomes as codes; every member **remains** `closure_sealed` |
| `ACC1_CLOSURE_SEAL_INVALID` | 409 | High | Seal from a state other than `closing`; seal request not bound to the applied closure initiation. (A master seal with an unsealed master-directed child or a non-`closed` independent child is `ACC1_CLOSURE_FAMILY_INELIGIBLE`) |
| `ACC1_CLOSURE_NOT_READY` | 409 | High | **(R2-F01)** Seal refused: the pinned pre-seal readiness absent, `not_ready`, `unavailable`, stale, for another `closure_cycle`, superseded by a later row, or the apply-time verification's `journal_watermark` differs from the approved W_pre |
| `ACC1_CLOSURE_ABORT_INVALID` | 409 | High | **(R2, R3)** Abort without matching blocking machine evidence (for a master abort: evidence not the latest row of a family member — an independent child's or a non-member's evidence is not acceptable); abort of a `closed` row; abort of a master-directed child alone. (Abort is **not** refused because a LED-01 dependency is unsatisfied.) |
