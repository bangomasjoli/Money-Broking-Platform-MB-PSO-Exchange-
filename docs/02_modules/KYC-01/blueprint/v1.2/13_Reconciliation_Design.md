...
1. Pass outcome with untraced ownership branch scan.
2. Authorised party active in CLT-01 while KYC party outcome is not pass.
3. Vendor reliance/accreditation expired or suspended but result used.
4. Vendor result validity expired while outcome remains pass.
5. Proofing assurance method below required policy.
6. Evidence hash mismatch on read/export/outcome use.
7. KYC pass treated as AML clear in downstream combined CDD.
8. Verified identity hash basis mismatch with CLT-01.
9. EDD case approved without minimum required measures.

Critical findings:

1. Pass with untraceable ownership.
2. Active authorised party without pass outcome.
3. Evidence-store hash mismatch.
4. Unaccredited vendor result used in pass outcome.
5. KYC/AML boundary violation.
