/**
 * Request-context plugin — CLT-01's OWN copy (mirrors services/iam2/src/plugins/request-
 * context.ts / services/sec1/src/plugins/request-context.ts / services/cfg1/src/plugins/
 ...
 * request, and installs the standard error handler for both foundation `AppError` and CLT-01's
 * own `Clt1Error` (see lib/errors.ts — empty this phase, wired up now so later phases need no
...
/**
 * §3.7 durable idempotency baseline (reused pattern from FND/IAM-01/IAM-02/SEC-01/CFG-01):
 ...
 * /internal/clt1/health`, is a read, not a mutation) — kept available now so later phases'
 * mutating endpoints (application intake, profile amendment, class change) need no plugin
 * change.
 */
...
function normalize(err: unknown): UnifiedError {
  if (err instanceof Clt1Error) return err;
