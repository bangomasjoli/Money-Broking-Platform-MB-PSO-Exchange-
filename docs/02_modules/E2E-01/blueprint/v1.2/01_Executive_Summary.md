...
v1.1 resolved:
1. Global cross-module saga / compensation model.
2. Mandatory correlation ID spine.
3. Orphaned intermediate-state sweeper.
4. Decision-bundle coherence.
5. Point-in-time eligibility snapshot.
6. Global freshness windows and revocation-propagation SLA.
7. In-flight interrupt / hold / compensate rules.
8. Irreversible-leg quarantine rule.
9. End-to-end per-correlation value conservation.
10. TRD / LED residual seam reconciliation.
11. Chainwide AIX net position zero proof.
12. Cross-module freeze propagation.
13. In-flight quiescence by stage.
14. Orchestrated recovery resume gate.
15. SEC expected-vs-emitted event manifest.
16. Joint evidence bundle by correlation ID.
17. Deposit / withdrawal rail boundary marked pending.

v1.2 is a final rollup only with no substantive control change.
