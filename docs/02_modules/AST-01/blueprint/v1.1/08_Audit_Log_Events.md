# AST-01 — 08 Audit, Evidence and Retention (v1.1)
...
| `ast1.instrument.lineage_assigned` / `lineage_forced` | Lineage set at creation; **forced** by on-chain continuity or declared predecessor [F06] | H |
...
| `ast1.hold.system_placed` | Integrity sweep / synthetic-in-PRODUCTION / standard retired / backstop trip — immediate, no human approval | **C** |
...
| `ast1.eligibility.hard_rule_backstop_triggered` | The **SQL backstop (reads the ledger)** rejected an `allow` insert — **means application logic was wrong**; a SYSTEM hold is placed | **C** |
...
| `ast1.integrity.lineage_gap_found` | Same on-chain identity present outside a shared lineage | **C** |
