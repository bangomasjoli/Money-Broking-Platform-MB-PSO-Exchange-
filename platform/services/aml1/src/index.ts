/**
 * AML-01 bootstrap. Loads config (fail closed on missing critical config), initialises the DB
 * pool, builds the app, and listens. Any startup failure exits non-zero. Mirrors
 * services/clt1/src/index.ts / services/cfg1/src/index.ts / services/sec1/src/index.ts /
 ...
 * schema — same precedent CFG-01's and CLT-01's own Phase 0 set), but no AML-01 route this phase
 * queries the database — there is no `aml1.*` schema yet (Phase 1 introduces it).
 */
import { closePool, initPool } from "@aix/foundation";
import { loadAml1Config } from "./config.js";
import { buildApp } from "./server.js";

async function main(): Promise<void> {
  const config = loadAml1Config(process.env);
  ...
  app.log.info(
    ...
    "aml1_service_started",
  );
}

main().catch((err) => {
  // Startup must fail loudly and exit non-zero; do not start in a degraded state.
  console.error("AML-01 startup failed:", err instanceof Error ? err.message : err);
  process.exit(1);
});
