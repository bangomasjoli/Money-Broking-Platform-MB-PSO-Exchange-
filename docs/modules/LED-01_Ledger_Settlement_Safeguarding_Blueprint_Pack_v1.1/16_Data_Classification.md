...
| Data | Classification | Protection |
|---|---|---|
| Live balance | Financial Critical / Security Critical | Versioned, locked, access restricted |
| Atomic reservation | Financial Critical | Audit retained |
| DvP settlement group/leg | Financial Critical | Settlement control |
| Backing position/encumbrance | Regulatory Critical | Safeguarding evidence |
| Conversion event | Financial Critical | Fee/rate/residual audit |
| Residual policy/account | Financial Critical | Bound-monitored |
| Journal hash-chain | Security Critical | Tamper evidence |
| External anchor | Security Critical | Integrity evidence |
| Clawback case | Financial Critical | Exception/reversal control |
| Operational account bound | Security Critical | Exposure control |

Rules:

1. Live balance is authoritative for reservation.
2. Balance snapshot is not authoritative for reservation.
3. Journal hashes and anchors are security-critical.
4. Backing encumbrance data is safeguarding-critical.
5. Residual account data cannot be treated as AIX profit evidence.
