...
| R-6 | Every `closed` account has zero LED-01 balance, no open hold/reservation/settlement (replay of the closure attestation against current LED-01 data) | ACC-01 ↔ LED-01 | Critical |
...
| R-8 | No non-`closed` subaccount under a `closed` master; no `is_default` subaccount `closed` while its master is not | ACC-01 only | High |
