...
| `iam2.approval_payload_hash_created` | Approval payload hash captured | High |
| `iam2.approval_payload_hash_mismatch` | Execution payload mismatch | Critical |
| `iam2.decision_token_issued` | Decision token issued | Medium |
| `iam2.decision_token_stale_blocked` | Stale decision token blocked | Critical/High |
| `iam2.protected_action_registered` | Protected action registered | High |
| `iam2.unregistered_sensitive_action_blocked` | Unregistered sensitive action blocked | Critical |
| `iam2.sod_risk_acceptance_requested` | SoD risk acceptance requested | High |
| `iam2.sod_risk_acceptance_approved` | SoD risk accepted | Critical/High |
| `iam2.sod_matrix_version_changed` | SoD matrix changed | Critical |
| `iam2.sod_matrix_integrity_failed` | Matrix integrity reconciliation failed | Critical |
| `iam2.break_glass_whitelist_violation` | Break-glass requested non-whitelisted permission | Critical |
| `iam2.cfg01_interim_lock_used` | Interim licence-lock source used | Medium |
| `iam2.sec01_interim_audit_index_used` | Interim local audit index used | Medium |
