...
## 5. Atomic Revalidate-And-Transmit

```mermaid
flowchart TD
  A[Instruction Built] --> B[Enter Send Critical Section]
  B --> C[Recheck WLT/AML/CFG/IAM/LED]
  C --> D{Fresh + Reserve Pinned + Not Revoked?}
  D -->|No| E[Fail Closed No Send]
  D -->|Yes| F[Acquire Reserve Send Lock]
  F --> G[Transmit to Provider]
```

## 6. Outbound Value Conservation

```mermaid
flowchart TD
  A[LED Reserve] --> B[Provider Amount Sent]
  B --> C[Rail/Network Fee]
  C --> D[Residual]
  D --> E{Reserve = Sent + Fee + Residual?}
  E -->|No| F[Critical Break]
  E -->|Yes| G[Notify LED]
```

## 7. Batch File Envelope

```mermaid
flowchart TD
  A[Batch Envelope] --> B[Item 1]
  A --> C[Item 2]
  A --> D[Item N]
  B --> E[Per-Item Finality]
  C --> E
  D --> E
  A --> F[Checksum + Manifest]
```
