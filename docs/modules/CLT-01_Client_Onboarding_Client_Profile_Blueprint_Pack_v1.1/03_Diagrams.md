...
```mermaid
flowchart TD
  A[Submitted Application] --> B[KYC/KYB Outcome]
  A --> C[AML/Sanctions Outcome]
  A --> D[PEP/Adverse Media Outcome]
  A --> E[Risk Rating]
  B --> F{All Satisfactory?}
  C --> F
  D --> F
  E --> F
  F -->|No| G[Block / Hold / Reject]
  F -->|Yes| H[IAM-02 Final Approval]
```
...
```mermaid
sequenceDiagram
  participant D as Downstream KYC/AML/Risk
  participant CLT as CLT-01
  participant IAM2 as IAM-02
  participant SEC as SEC-01
  D->>CLT: Hit / expiry / risk change
  CLT->>CLT: Restrict or suspend
  CLT->>IAM2: Revoke/revalidate authority
  CLT->>SEC: Audit status feedback
```
