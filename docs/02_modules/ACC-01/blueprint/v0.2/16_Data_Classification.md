...
| Status history, closure attestations (incl. `seal_version_observed`, journal watermark) | Restricted / Security Critical | Append-only evidence; **no hard deletion** — retained per the platform/client-record retention policy once defined (ACC-01 sets none) |
