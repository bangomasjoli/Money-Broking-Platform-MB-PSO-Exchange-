...
4. `action` must include the endpoint/action namespace, for example `module.endpoint.operation`, so the same key cannot collide across unrelated actions.
5. Replays from a different actor or service identity are treated as a different idempotency scope and must not return another actor's result.
6. Where a workflow requires cross-actor approval, idempotency scope must include workflow reference and approval action type.
...
| scheduled_job | Life of job registration + records policy |
| job_run | Operational/audit evidence retention |
| job_queue_message | According to job owner module and event criticality |
| rate_limit_decision_log | Security/operations retention |
