...
supersedes: v1.1, v1.2, v1.3 (archived)
baseline_commit: 1fa3168
---

# 01 Project Charter  
# AIX Institutional Digital Asset & RWA Platform

> ## v1.4 — RE-BASELINED ON APPROVED DOC 00 v1.4
>
> **Base document is now `00_Licence_Scope_And_Feature_Lock_v1.4.md` (APPROVED).** v1.3's
> normative scope is superseded prospectively; v1.3 is archived intact and its historical
> statements remain accurate for the period they describe.
>
> **What changes:** the platform is re-stated as an institutional digital asset and RWA
> platform with **four products** — AIX Spot, AIX OTC, AIX Pay, AIX RWA — over one shared
> institutional core, with Spot executing under **Model A** (externally-routed agency
> execution).
>
> **What does not change:** every accepted governance control. Compliance-first delivery,
...
> fail-closed behaviour are **preserved in full**. No control is weakened to make product
> delivery easier.
>
> **Derived from:** Doc 00 v1.4; `DEC-011` (account hierarchy); `DEC-012` (Spot execution
> architecture); `STR-01`; `STR-02`.
>
> **Approves nothing Doc 00 does not.** No securities activity, RWA issuance, venue, order type
> or asset is activated by this Charter. Unresolved regulatory questions remain fail-closed
> (Doc 00 §23).
...
| Item | Details |
|---|---|
| Document name | 01_Project_Charter_v1.4.md |
| Platform | AIX Institutional Digital Asset & RWA Platform |
| Document type | SDLC Phase 1 / Project Initiation |
| Version | v1.4 |
| Status | **APPROVED** — re-baselined on Doc 00 v1.4; internally reviewed for consistency against it |
| Prepared for | Management, product, compliance, development, architecture, and implementation planning |
| Base document | **00_Licence_Scope_And_Feature_Lock_v1.4.md (APPROVED)** |
...
**Product names** (Doc 00 v1.4 §2A — platform terminology, not regulatory permission):
...
| **AIX Exchange** | **Reserved** for securities / financial-instrument Exchange capability | Securities / Exchange — scope unresolved |
...
**"AIX Exchange" is reserved terminology, not an approval.** Reserving it grants no securities
capability and asserts nothing about the scope of AIX's Exchange approval, which is unresolved
(Doc 00 §12C, `R1-Q1b`).
...
**The platform must not be developed as a venue.** Two distinct boundaries (Doc 00 v1.4 §6):

- **Standing prohibition, not unlocked by any approval:** an AIX-operated matching book for digital currency, client-to-client crossing, market making, principal liquidity (Model C, §12A.2).
- **Pending applicable approval:** securities / financial-instrument Exchange capability, whose scope is unresolved (Doc 00 §12C).

---

## 4. Project Vision

The project vision is to build a secure, scalable, modular, and audit-ready **institutional digital asset and RWA platform**, operating under AIX's Labuan Money Broking and PSO licences, with securities capability designed but gated behind classification and applicable approval.

The platform must be built as a real production platform, not a demo.

**One institutional core, four products.** The four products share a single core rather than operating as four systems (§12A). A product may exist architecturally while individual capabilities remain disabled pending applicable regulatory approval — the product-architecture / regulatory-permission separation is Doc 00 v1.4 §1.A and governs this Charter throughout.
...
9. **Four product pillars** — Spot, OTC, Pay, RWA — over shared infrastructure.
10. **Provider-neutral external execution** — no named venue is assumed or approved.
11. **Capability gating** — every capability independently controllable and fail-closed.
12. Future securities / Exchange capability without rebuilding from zero, subject to approval.
...
Derived from Doc 00 v1.4, `DEC-011` and `DEC-012`. **Architecture, not activation** — every capability remains subject to Doc 00 §21's gates.
...
**PSO boundary governs.** The Doc 00 §5.2 baseline is approved scope; **every capability beyond it is `PENDING / FEATURE-LOCKED`** until its PSO-scope basis is verified (Doc 00 §12D). A public checkout surface additionally requires its own perimeter analysis before exposure. Pay consumes the shared ledger, compliance and reconciliation core; it does not duplicate them.
...
**Classification precedes everything.** Security / security-token route and non-security route are distinguished; **unresolved classification fails closed** (Doc 00 §12B). The securities route remains locked pending `R4-Q2`. RWA reuses the shared core and must not re-implement identity, compliance, ledger or settlement.
