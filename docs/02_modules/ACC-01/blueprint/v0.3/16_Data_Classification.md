...
| Status history, **closure readiness, closure attestations, closure recovery rows** (incl. `seal_version_observed`, watermarks, resolution-version evidence, in-flight status, balance *state*) | Restricted / Security Critical | Append-only evidence; **no amounts** (states/counts/codes only); **no hard deletion** — retained per the platform/client-record retention policy once defined (ACC-01 sets none) |
...
| Dependency evidence records (`DEP-*`, `provider`, contract refs) | Restricted | Configuration evidence; contains no secret; the dedicated peer credentials are Secret |
