...
| **`LINEAGE_SECURITY_REVIEW_REQUIRED`** [F20] | 32 | `NOT_ASSESSED` | A real `SECURITY` determination newer than this instrument's own record exists in its lineage or its underlying's lineage; cleared only by a newer elevated record |
...
|---|---|---|
| `AS001` | Backstop denied an `allow` insert or a conjunct-row approval (`backstop_permits` false) | deny + `hard_rule_backstop_triggered`; `SECURITY_INSTRUMENT_NOT_ADMISSIBLE_TO_MB_PRODUCT` where B1 |
| `AS002` | Immutable column/table touched (`UPDATE` of an insert-immutable key, token binding column, `deployment_environment`, lineage field) | `AST1_INSTRUMENT_IMMUTABLE_FIELD` or internal error + critical event |
| `AS003` | Token consumption rejected by `assert_token_consumable()` | `AST1_DECISION_STALE`; `consume_backstop_triggered` when the app had not already flagged it |
| `AS004` | Identity/lineage invariant (duplicate canonical identity, underlying cycle, class/form inconsistency) | `AST1_INSTRUMENT_DUPLICATE_IDENTITY` / `AST1_UNDERLYING_CYCLE` / validation error |
