...
| Platform strategy analysis (no authority over masters) | [05_strategy/](../05_strategy/) — `STR-01` re-baseline analysis and `STR-02` decision pack, both DRAFT. `STR-02`'s `DEC-REQ-A2` is approved as `DEC-011`; its other decisions are not approved |
