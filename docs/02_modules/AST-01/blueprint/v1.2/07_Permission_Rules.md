# AST-01 — 07 Permission Rules (v1.2)
...
| `ast1.classification.approve_elevated` | `MLRO` | Second checker on the `elevated` path — including the **reaffirmation of a sibling** held by `LINEAGE_SECURITY_REVIEW_REQUIRED` and a wrapper of a security-determined instrument |
...
| `LINEAGE_MERGE` | HUMAN | compliance M; `COMPLIANCE_OFFICER` C | No row | **DCR-AST1-006** |
...
| `CLASSIFICATION_APPLY` **reaffirming a sibling** after a lineage `SECURITY` determination **[v1.2]** | HUMAN | as elevated `CLASSIFICATION_APPLY` (M + `COMPLIANCE_OFFICER` + `MLRO`) | as elevated row | **DCR-AST1-006** |
