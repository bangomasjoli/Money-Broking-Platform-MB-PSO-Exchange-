...
- Baseline commit: `5a4f872` — `feat(cfg1): enforce environment capability availability (MIG-004)` (parent `bdc5dfc`; evidence recorded at `c3ac87d`). The last UI commit is unchanged: `bff15e8` — `feat(ui): add admin approval queue` (UI Phase 2R). This is the front matter `baseline_commit`: the newest non-governance commit this file was verified against, and the current accepted baseline. It is not the repository HEAD, which this file never records; the conductor reads HEAD from Git. Only control-layer records (this file, the registers, `tasks/**`) follow it. The last accepted **backend** implementation commit is `5a4f872` (`MIG-004`; see §4). Before it: `5f78a0b` (`MIG-005`), then `9b0bab7` — `fix(perf): harden IMP-02 measurement evidence validation`.
- Migration head **071** (`platform/infra/migrations/071_cfg1_environment_scope.cjs`, 71 migrations, `MIG-004`), also stated in [OPEN_FINDINGS.md](../OPEN_FINDINGS.md) (FND-01 Shared Rate-Limit Engine, migration 070).
...
None. **MIG-004** (CFG-01 `environment_scope` + live decision-chain step 4) is **ACCEPTED** at `5a4f872`, with controlled carry-forwards. Independent GPT review: ACCEPT WITH CONTROLLED CARRY-FORWARDS; human acceptance by Aiman. Records: [tasks/MIG-004/](../03_implementation/tasks/MIG-004/). **Closes `CFG-FIND-001`** (HIGH) and **the CFG-01 portion of `FND-FIND-013`** (the row stays OPEN for the WLT-01 portion). Human decisions HD-1…HD-4 all **APPROVED**: `current_state` = product/operational activation conjunct, recorded as **DEC-014**; canonical PRODUCTION denies `production_activation_absent` until production-activation state exists (state 3 remains unbuilt); the `feature` config-integrity seal is left unextended in MIG-004 and its decision-time verification gap is tracked separately as **`CFG-FIND-002`** (MEDIUM, unchanged); a structural `exchange.` prefix deny is added at evaluation. Decision precedence: prohibited-registry deny precedes the caller-environment-mismatch check, so a permanently prohibited request always denies as `prohibited` even with a mismatched environment. Accepted observations: **MIG-004-O1** — a pre-existing cross-file CLT-01 test-isolation race (`clt1-db.test.ts` unscoped `DELETE FROM clt1.client_profile` vs. concurrent inserts in `clt1-principal-membership-route.test.ts`), reproduced independently on the untouched baseline; ruled REVIEW REQUIRED / separate CLT triage, not a MIG-004 defect. **MIG-004-O2** — after migration 071 every `environment_scope` entry defaults DISABLED with no write path to change one, so no capability becomes more available; ruled an accepted design consequence, not a defect. No capability enabled; no `environment_scope` write path exists; `MIG-004` makes nothing production-ready.

Before it, **MIG-005** (canonical five-environment model in `@aix/foundation`; `demo` added; `staging` and unknown → PRODUCTION) is **ACCEPTED** at `5f78a0b`, with controlled carry-forwards. The review ran in the same session as the implementation; the human accepted knowing this. Records: [tasks/MIG-005/](../03_implementation/tasks/MIG-005/). Carry-forwards: `FND-FIND-012`, `FND-FIND-013` (WLT-01 portion), `WLT-FIND-016`, plus governance cleanup G-1 (record the staging mapping as decided in the masters, plus the Doc 00 §25.2 `MIG-004` wording, R-5). No capability enabled, no migration added.
...
- **MEDIUM:** `CFG-FIND-002` — the CFG-01 `feature` config-integrity seal is never verified at decision time, and its hash does not cover `environment_scope`; trigger: before UAT/DEMO use of any environment-gated capability, or any `environment_scope` write path. Raised at `MIG-004` plan approval; deliberately not fixed inside `MIG-004` (its own compensating control — a live verify-time re-read — exists meanwhile); own remediation task required.
...
- **Closed:** `CFG-FIND-001` (`MIG-004`, commit `5a4f872`); `FND-FIND-013` CFG-01 portion (`MIG-004`, commit `5a4f872` — row stays OPEN for the WLT-01 portion); `IMP-02-FIND-010` and `IMP-02-FIND-011` (`IMP02-MA-HARDEN-001`, commit `9b0bab7`); the Turn M-B gate on them is satisfied. Turn M-B has not started.
...
- **Governance / masters:** the `DEC-013` core re-baseline is complete (§7A). The stated next phase is **module blueprints**: create `EXM-01`, `EXO-01`, `EXC-01`, `EXP-01`; update the blueprints Doc 00 §25.2 lists as affected. **The re-baseline authorises no application code**; the `MIG-001`…`MIG-010` migrations each require their own approved task record. `MIG-005` is **ACCEPTED** (`5f78a0b`) and `MIG-004` is **ACCEPTED** (`5a4f872`, [tasks/MIG-004/](../03_implementation/tasks/MIG-004/)). The next Phase-A migration or controlled turn is not yet decided — a human decides, with `CFG-FIND-002` (own task) a prerequisite before any UAT/DEMO use of an environment-gated capability. Nothing else is started.
