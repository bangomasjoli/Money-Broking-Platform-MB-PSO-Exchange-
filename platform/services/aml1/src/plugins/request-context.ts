/**
 * Request-context plugin — AML-01's OWN copy (mirrors services/clt1/src/plugins/request-
 * context.ts / services/cfg1/src/plugins/request-context.ts / services/sec1/src/plugins/
 * request-context.ts / services/iam2/src/plugins/request-context.ts; deliberately not imported
 * from any of them — F3(c) module-import-boundary). Builds a RequestContext at the edge, binds it
 * to async storage, decorates the request, and installs the standard error handler for both
 * foundation `AppError` and AML-01's own `Aml1Error` (see lib/errors.ts — empty this phase, wired
 * up now so later phases need no request-context change).
 */
 ...
 * sensitive state-mutating POSTs require an Idempotency-Key header. Missing/blank fails closed
 * before any DB work. Not used by any Phase 0 route (the one route this phase registers, `GET
 * /internal/aml1/health`, is a read, not a mutation) — kept available now so later phases'
 * mutating endpoints (screening request, match disposition) need no plugin change.
 */
...
function normalize(err: unknown): UnifiedError {
  if (err instanceof Aml1Error) return err;
