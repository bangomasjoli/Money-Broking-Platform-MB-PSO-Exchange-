...
| `ACC1_REAL_USE_NOT_PERMITTED` | 403 | High | A gated operation (G1–G6, file 01 §4.5) outside DEVELOPMENT/TEST (incl. unknown environment). Carries the gate id, never the prerequisite internals |
...
| `ACC1_CLOSURE_BLOCKED` | 409 | High | Attestation `blocked`/`unavailable`/stale/pre-barrier/`seal_version` mismatch/`in_flight_predating_seal > 0`, non-`closed` children, authority restriction active. Body lists attester outcomes as codes; target **remains** `closure_sealed` |
| `ACC1_CLOSURE_SEAL_INVALID` | 409 | High | Seal from a state other than `closing`; master seal with a non-`closed` child; seal/complete not bound to the applied closure request |
