# AST-01 — 08 Audit, Evidence and Retention (v1.2)
...
| `ast1.asset.class_corrected` | Governed `ASSET_CLASS_CORRECTION` applied; earlier records collapse [AST-P-3] | **C** |
| `ast1.lineage.merged` | Governed, irreversible merge | **C** |
| `ast1.lineage.security_determination_propagated` | A real `SECURITY` record applied: affected sibling/wrapper instruments now fail closed (`LINEAGE_SECURITY_REVIEW_REQUIRED`); tokens revoked (metadata: affected count, ids) [F20] | **C** |
...
| `ast1.eligibility.lineage_security_review_required` | Deny by conjunct C0b [F20] | M |
...
| `ast1.integrity.lineage_gap_found` | Canonical-identity uniqueness breached in restored/tampered data; or a current real `NON_SECURITY` record whose transitive underlying reaches a `SECURITY` lineage lacks `UNDERLYING_LINEAGE` in `elevated_basis` [F19] | **C** |
