# AST-01 — 09 Error Handling

**Status: PLANNED / AWAITING REVIEW.** Two vocabularies, kept separate as in CFG-01:
...
| `INSTRUMENT_ON_HOLD` | ≤ 48 | `NOT_ASSESSED` | Active hold |
...
| `RWA_ROUTE_REGULATORY_QUESTION_OPEN` | ≤ 48 | `NOT_ASSESSED` | Non-security RWA on MB rails / secondary (`R4-Q6`/`R4-Q7`) |
...
| `AST1_CLASS_CLASSIFICATION_CONFLICT` | 422 | Presumptive-securities class proposed `NON_SECURITY` (HD-4) |
...
| `AST1_CLASSIFICATION_SELF_APPROVAL_FORBIDDEN` | 403 | maker = checker at any layer |
| `AST1_CHECKER_CONFLICT` | 403 | Checker attached evidence to the case, or lacks the elevated role (HD-8) |
...
| `AST1_DECISION_BINDING_MISMATCH` | 409 | Environment/payload binding mismatch (same name family as `CFG1_DECISION_BINDING_MISMATCH`) |
| `AST1_VERSION_CONFLICT` | 409 | Stale `expected_version` |
| `AST1_HOLD_PLACEMENT_FORBIDDEN` | 403 | Actor lacks `ast1.hold.place` (HD-6) |
| `AST1_ATTESTATION_INVALID` | 422 | `EXM-01` attestation for a non-security instrument |
