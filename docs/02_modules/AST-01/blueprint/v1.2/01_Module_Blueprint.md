# AST-01 — 01 Module Blueprint (v1.2)

**Status: REMEDIATED / AWAITING RE-REVIEW.** Nothing here is accepted or approved for implementation. v1.0 (`78ec1e4`) and v1.1 (`e20b8ca`) are reviewed historical evidence (both `REMEDIATE`). Reviews: [`04-review.md`](../../../../03_implementation/tasks/AST-01/04-review.md) (v1.0), [`04-review-r2.md`](../../../../03_implementation/tasks/AST-01/04-review-r2.md) (v1.1). Finding-by-finding records: [`05-remediation.md`](../../../../03_implementation/tasks/AST-01/05-remediation.md) (v1.1), [`05-remediation-r2.md`](../../../../03_implementation/tasks/AST-01/05-remediation-r2.md) (v1.2). Changes from v1.1 are tagged **[v1.2: Fnn]**.
...
| **INV-17** | **Lineage.** Any prior real `SECURITY` determination anywhere in an instrument's lineage, **or in the lineage of an instrument it wraps**, triggers the elevated approval path for any later loosening; it cannot be avoided by an interim `UNRESOLVED`, a replacement instrument/asset under continuity, or by editing lineage fields (immutable from creation). **A `SECURITY` determination also immediately fail-closes every linked `NON_SECURITY` sibling** until an explicit elevated review (§4.7A) [F19, F20] | Lineage tables, trigger-computed; derived conjunct; SQL B8; §4.7, §4.7A |
...
| **INV-20** | **Consumption is serialised with mutation.** Mint and consume each run in one transaction under one lock order (instrument before token); the SQL backstop re-runs at the consuming `UPDATE`; token binding columns are immutable [F18] | 05 §7, §7A; T-CON-* |
...
**Class correction [AST-P-3].** An asset explicitly labelled `SECURITY` or `SECURITY_TOKEN` cannot receive a `NON_SECURITY` outcome while it keeps that contradictory label. `asset_class` is **frozen once any instrument of the asset exists**; the only change is the governed **`ASSET_CLASS_CORRECTION`** (maker `INSTRUMENT_CLASSIFIER`; when it moves a class **out of** `SECURITY`/`SECURITY_TOKEN`, `COMPLIANCE_OFFICER` **and** `MLRO` attested — it loosens). It preserves the lineage (no lineage field changes), never crosses the fiat boundary once an instrument exists (`FIAT_CURRENCY` ⇔ `instrument_form = 'FIAT'`, [F22]), and recomputes the cached fingerprint, so every earlier record collapses to `UNRESOLVED` (`CLASSIFICATION_IDENTITY_DRIFT`) until a new governed classification is made — on the elevated path where the lineage holds a `SECURITY` determination. The alternative is a **replacement contract** (new address) under lineage (§3.9). Same-contract re-registration is impossible (§3.9 E).
...
`network_registry` read-only at runtime; a chain/network absent from it cannot carry an instrument. Contract addresses are canonicalised — by one function shared by registration and resolution — before the uniqueness check and before every resolution (§3.11). This does **not** replace WLT-01's `chain_coverage`; consumers AND both (DCR-AST1-002).
...
- **Candidate surfacing (advisory).** At submission AST-01 lists **lineage candidates** to the checker — same issuer reference, shared underlying instrument, similar name — and the approval payload binds the candidate list and carries the attestation "lineage reviewed". A candidate the checker judges continuous is linked by a governed lineage merge. Candidates are advisory; the deterministic signals above are not.
- **F20. Siblings.** A real `SECURITY` determination anywhere in a lineage fail-closes every other real `NON_SECURITY` instrument linked to it, immediately, through a **derived conjunct** (§4.7A). Classification remains per instrument and is never rewritten.
...
2. the evidence bundle must contain **≥ 1 item recorded after `recorded_at` of the newest such `SECURITY` record** (across every basis lineage), and **no item whose `content_sha256` appears in that record's bundle** counts toward the requirement;
...
| new instrument under a replacement asset | Declared predecessor + same-asset membership + lineage merge (§3.9); residual AR-19 |
...
### 4.7A Lineage-linked security determination — a fail-closed conjunct (F20)

Lineage asserts economic-subject continuity, but `elevated` guards only *future loosening*. If sibling *B* was `NON_SECURITY` before sibling *A* was determined `SECURITY`, B must not simply carry on.

**Rule.** A real instrument whose current record is `NON_SECURITY_DIGITAL_ASSET` derives **`NOT_ASSESSED`, `LINEAGE_SECURITY_REVIEW_REQUIRED`** (conjunct C0b) for **every** subject, and the SQL backstop denies (B8), whenever a real `SECURITY` record that is **newer than its own current record** exists on (a) another instrument of its lineage root, or (b) an instrument in the lineage of any transitive underlying instrument.

- **Immediate.** It is derived from the ledger at read time, so it takes effect at the commit of the triggering `SECURITY` record, with no row to write, forget or race. Outstanding tokens of the affected instruments are also **revoked in the same transaction** (`lineage_security_determination`); independently, a surviving token is rejected at consumption because the conjunct is recomputed there (05 §7A).
- **Not a reclassification.** No sibling record is rewritten and none is silently made `SECURITY`; legal classification stays per instrument (AST-HD-9). It is a safety conjunct based on lineage history (the AST-HD-6 model): it only denies.
- **Cleared only by explicit elevated review.** The sibling receives a **newer** record through the **elevated governed path** (two attested checkers, new post-`SECURITY` evidence, "lineage reviewed"): `NON_SECURITY` to reaffirm (a `RECLASSIFICATION` case whose proposed outcome equals the current one), `SECURITY`, or `UNRESOLVED`. Because the lineage now holds a `SECURITY` determination, no non-elevated record can be appended; nothing else — no hold release, admission, flag, role or configuration — clears it.
- **Serialised.** The `SECURITY` record's apply locks every affected instrument (siblings and wrappers), ascending, before it writes; a consumer of a sibling token either commits first or sees the determination (05 §7A).
- **Scope.** Real instruments only; a synthetic `SECURITY` never triggers it. An instrument that is itself `SECURITY` is already `INELIGIBLE` for MB/PSO by the hard rule. **Extension beyond F20's wording, flagged for the re-reviewer:** instruments that **wrap** a security-determined instrument (via the underlying link) are treated like siblings, so the `UNDERLYING_LINEAGE` elevated basis (§4.7) has no post-hoc gap.
...
1  E := effective outcome (§4.4 collapses to UNRESOLVED, incl. a real NON_SECURITY without a production-applicable basis)
...
5  conjuncts, each only denies:
     ...
     C0b lineage security   → NOT_ASSESSED  LINEAGE_SECURITY_REVIEW_REQUIRED   (§4.7A)
