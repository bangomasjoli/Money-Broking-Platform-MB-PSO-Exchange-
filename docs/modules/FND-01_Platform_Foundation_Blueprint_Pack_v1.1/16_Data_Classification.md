...
| Scheduled job metadata | Confidential | Reveals operational cadence |
| Job run metadata | Confidential / Financial Critical where money action | Depends owner module |
| Job payload references | Confidential / Restricted | Payload itself should be referenced, not inline |
| Rate-limit decision logs | Confidential / Restricted | May include security metadata |
| DB grant/RLS evidence | Security Critical | Reveals access model |
| Async correlation IDs | Internal / Confidential | Trace identifiers |
...
## 7. Scheduler / Job Queue Data Handling

Rules:

1. Job payloads should be stored by reference where sensitive.
2. Job queue metadata must carry correlation ID.
3. Dead-letter evidence must not contain secrets or full PII.
4. Financial or compliance job data inherits the classification of the owner module.
5. Scheduled job definitions for safeguarding/reconciliation are Confidential or higher.

---

## 8. DB Isolation Evidence Handling

DB grant, RLS, and access-test evidence is Security Critical.

Rules:

1. Access restricted to Technology, Security, Auditor, and approved reviewers.
2. Evidence export requires approval.
3. Evidence must not expose production credentials.
4. RLS policy details may be shared only with approved technical/security reviewers.
