...
stateDiagram-v2
  ...
  scheduled --> missed
  missed --> incident_or_alert
  completed --> [*]
```

Rules:

1. Critical missed run creates Critical alert.
2. Job run must be idempotent.
3. Job state must include correlation ID.

---

## 8. Background Job Queue State
...
3. Dead-letter must be visible and auditable.
4. Worker restart must not duplicate side effects.

---

## 9. Rate-Limit Decision State
