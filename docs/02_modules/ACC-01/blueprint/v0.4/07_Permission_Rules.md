...
| `acc1.master_account.close_initiate` | master_account | close_initiate | financial_critical | **false** | **(R3, ACC-R3-HD-02)** Closure **initiation**: **maker-only, entitlement-checked, audited** (master + default + every operational child → `closing`). **Non-approval on purpose**: it reaches guard step 10, a real entitlement; an approval-gated code would return `approval_required` at step 7 for any actor (`IAM2-FIND-002`) and could not be an entitlement. Replaces v0.3 `acc1.master_account.close` |
| `acc1.master_account.close_seal` | master_account | close_seal | financial_critical | **true** | **(R2)** `seal_closure`: the checker's approval is the **ONE final human approval** immediately before sealing, bound to the exact pinned readiness (R3) |
| `acc1.master_account.close_abort` | master_account | close_abort | financial_critical | **true** | **(R2)** `abort_closure`: governed recovery, maker-checker only; **(R3)** a master abort returns the whole family (never an independent closure) and needs no LED-01 dependency |
...
| `acc1.master_account.close_complete` | master_account | close_complete | financial_critical | false | **(R2)** Machine-verified completion CAS. **Non-approval ⇒ a real entitlement**; no second checker. **(R3)** For a master this is the atomic family completion |
...
| `acc1.subaccount.close_initiate` | subaccount | close_initiate | financial_critical | **false** | **(R3)** Initiation of an **independent** closure of a non-default subaccount, maker-only as above. The default never has an independent closure (it closes only with its master, atomically) |
| `acc1.subaccount.close_seal` | subaccount | close_seal | financial_critical | **true** | **(R2)** Final human approval before sealing |
| `acc1.subaccount.close_abort` | subaccount | close_abort | financial_critical | **true** | **(R2)** Governed recovery; **(R3)** only for an **independent** closure — never for the default, and never for a master-directed child (those return only with their master) |
| `acc1.subaccount.close_collect_evidence` | subaccount | close_collect_evidence | sensitive | false | **(R2)** As master |
| `acc1.subaccount.close_complete` | subaccount | close_complete | financial_critical | false | **(R2)** As master; **(R3)** independent subaccounts only |
...
- **Entitlement honesty (R2-F01 item 6; R3-F06):** the approval-gated codes (`.create`, `.close_seal`, `.close_abort`, `.restriction.*`) are **not entitlement checks** under current IAM-02 (§3). The **non-approval** codes (`*.close_initiate`, `*.close_collect_evidence`, `*.close_complete`, `*.read`, `*.update_profile`) **are** genuine step-10 entitlement checks at the guard. **(R3, ACC-R3-HD-03)** ACC-01 does not rest on that alone for the safety-critical steps: real initiation, seal and abort additionally require the **attested entitlement evidence** returned by IAM-02 per call (`DEP-IAM-ENTITLEMENT`, file 01 §4.5) — a guard 'allow' without the attested proof, or `approval_required`, is not sufficient.
- **Closure initiation is not maker-checker (ACC-R3-HD-02).** The catalogue therefore carries **no approval-gated initiation code**. The Role Matrix / IAM-02 change that assigns who may hold `*.close_initiate` and exposes the attested maker-initiation seam is **DCR-ACC-IAM-07** / **DCR-ACC-GOV-02** (ACC-01 assigns no role, ACC-HD-2). Until it exists, real closure initiation is externally gated.
...
2. `IAM2-FIND-002` is a **hard prerequisite** before any real-actor ACC-01 governed apply or closure initiation (`DEP-IAM-ENTITLEMENT`, file 01 §4.5 — satisfied only by IAM-02's per-call entitlement evidence, never by a declaration that the finding is fixed), and the **actor-binding** gap is a second, separate prerequisite (`DEP-IAM-ACTOR-BINDING`, DCR-ACC-IAM-06, §4.1). Its platform-wide dimension — the step-7 short-circuit affects **every** approval-gated permission of every module, CLT-01 included — should be considered when the `IAM2-FIND-002` governance row is next revised (recorded here; `OPEN_FINDINGS.md` is **not** modified by this task).
...
1. **Maker ≠ checker** — enforced by IAM-02 at approval (`IAM2_SELF_APPROVAL_BLOCKED`); it applies to **seal, abort, create and restriction changes**. **Closure initiation has no checker** (ACC-R3-HD-02). **(R2-F04) ACC-01 does not claim that apply is bound to the maker under today's seam.** `execute-verify` takes a caller-supplied `actor_id`; sending `actor_id = stored requested_by` proves nothing, because any holder of the decision token passes it. Real governed apply requires IAM-02 to **authenticate the apply actor independently of ACC-01** and return attested facts (`DEP-IAM-ACTOR-BINDING`, DCR-ACC-IAM-06, ACC-R2-HD-07): actor-assertion authority, approval id, authenticated actor, maker, checker, policy id, entitlement evidence, payload hash, action, resource, entity/client scope. ACC-01 then requires authenticated actor = maker = stored `requested_by`, checker ≠ maker, and records the attested values. Until that exists ACC-01 cannot see the approver and does not invent the proof; the recorded `approval_id` is labelled `caller_asserted`. *Local defence in depth (not a substitute):* the IAM-01 session principal must equal `requested_by` at apply and at cancel.
...
4. **Who may be maker and checker** for each change type — and who may be the **initiating maker** of a closure — is **not defined by ACC-01** (ACC-HD-2). Existing Role Matrix rows relevant as *sources*: §7 "Close client account" and §23 "Account freeze/unfreeze". Rows for master-account/subaccount **create**, the **maker-only initiation** of closure, and the maker/checker for **seal**, **abort**, **restrict** and **lift** as applied to accounts, must be governed in the Role Matrix + IAM-02 (**DCR-ACC-GOV-02**). Until then no real-actor governed apply is permitted (`DEP-IAM-ENTITLEMENT`).
5. **Emergency restrict-only single-actor path:** not in this version (OQ-06/HD-4, recommended); WF-26 requires maker-checker. Any such path would be a later human decision, restrict-only, never able to lift or activate. **(R3, honest interaction with HD-4):** a maker-only closure initiation puts a target in `closing`, which halts client activity outside the drain allow-list, so an entitled maker can by itself apply a *de facto* restriction. ACC-R3-HD-02 accepts this; HD-4 is **not** decided by it. The mitigations are the entitlement requirement, the Critical audit, the fact that `closing` cannot seal without the checker's final approval, and the maker-checker governed abort that undoes it.
...
### 4.1 Target IAM-02 contract requested (DCR-ACC-IAM-06, -05, -07 — external, not decided by ACC-01)

**Provenance of the actor (R3-F04).** An apply-time verification that accepts an **actor-authentication reference produced outside ACC-01** — an IAM-01 authenticated-session reference, a recent-auth reference, or an IAM-01-produced signed/verifiable identity assertion — which **IAM-02 itself validates through its own trusted seam with IAM-01**. It returns, as one attested record: `actor_assertion_authority`, `authenticated_actor_id`, `maker_user_id`, checker/approver identity, `approval_id`, `policy_id`, **entitlement/grant evidence** (per role: evaluated, actor, permission code, decision, grant reference, policy, time), `payload_hash`, `action`, `resource`, `entity_id`/`client_id`, and `credential_scope`. ACC-01 consumes the record and never a body assertion. **An assertion minted or signed by ACC-01 does not satisfy `DEP-IAM-ACTOR-BINDING`** — it would be ACC-01's own claim echoed back, the caller-asserted posture ACC-R2-HD-07 forbids. The seam is distinct from today's `execute-verify` so that a peer lacking it refuses **without consuming a token**.

### 4.2 Maker-initiation seam requested (DCR-ACC-IAM-07 — ACC-R3-HD-02)

The same attested-record contract, for a **non-approval** permission (`acc1.master_account.close_initiate`, `acc1.subaccount.close_initiate`): a per-call, IAM-02-verified `initiator` entitlement decision with the same actor provenance and grant evidence, and no approval or checker. The Role Matrix defines who may hold the permission (DCR-ACC-GOV-02). Until IAM-02 exposes it, **real closure initiation remains externally gated** (`DEP-IAM-ENTITLEMENT`, `DEP-IAM-ACTOR-BINDING`).
