...
## 5. v1.1 Implementation Handover Addendum

Before implementation, create a shared cross-module library or platform service for:

1. `correlation_id` generation and propagation.
2. saga step registry.
3. decision-bundle validation.
4. point-in-time eligibility snapshot.
5. expected SEC event manifest.
6. orphaned intermediate-state sweeper.
7. evidence bundle retrieval by correlation ID.

Recommended implementation sequence update:

1. Implement FND correlation and idempotency primitives first.
2. Implement SEC expected-event manifest support early.
3. Implement CFG/IAM decision-token patterns.
4. Implement E2E saga primitives before WLT/LED/TRD money movement.
5. Implement module-local controls.
6. Implement E2E tests after each money-flow module.
