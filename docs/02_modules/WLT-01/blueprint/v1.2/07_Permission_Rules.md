...
| `wlt1.destination.verify_and_consume` | Execution-time revalidation/consume |
| `wlt1.limits.manage` | Manage destination/client limits |
| `wlt1.inbound_source.screen` | Screen inbound source |
| `wlt1.quarantine.release` | Release quarantined inbound deposit |
| `wlt1.proof_of_control.verify` | Verify wallet proof-of-control |
| `wlt1.chain_coverage.manage` | Manage supported chain/provider coverage |
...
11. limit profile change.
12. first-use limit override.
13. quarantine release.
14. unhosted proof-of-control exception.
15. unsupported-chain exception.
16. address-poisoning review clearance.
...
1. Execution-time verify-and-consume cannot be bypassed by downstream module.
2. Limit breach cannot be overridden without approval and audit.
3. Quarantine release requires Compliance/Operations approval according to policy.
4. Unhosted wallet proof-of-control exception requires Compliance approval.
5. Third-party wallet/payout beneficiary approval must bind to KYC-01 identity evidence.
6. Unsupported chain cannot be marked clear by ordinary user.
7. Address-poisoning review clearance requires maker-checker.
