/**
 * Unit tests for CFG-01's own interim internal-identity guard
 * (services/cfg1/src/plugins/internal-identity.ts) — missing/wrong/correct token behaviour.
 *
 * Phase 0 registers exactly one route (`GET /internal/cfg1/health`), which is deliberately
 ...
 * `tests/unit/iam-import-boundary.test.ts` uses to prove its own scanner fires on a synthetic
 * violation. This is not a second business endpoint; it never ships.
 */
import { describe, expect, it } from "vitest";
import Fastify from "fastify";
import { registerRequestContext } from "../../services/cfg1/src/plugins/request-context.js";
import { makeCfg1InternalIdentityGuard } from "../../services/cfg1/src/plugins/internal-identity.js";

const EXPECTED_TOKEN = "test-cfg1-internal-token-scratch";

async function buildScratchApp() {
  const app = Fastify({ ajv: { customOptions: { removeAdditional: false } } });
  await registerRequestContext(app);
  const requireInternal = makeCfg1InternalIdentityGuard(EXPECTED_TOKEN);
  app.get("/scratch/internal-only", { preHandler: requireInternal }, async () => ({ ok: true }));
  await app.ready();
  return app;
}

describe("CFG-01 internal-identity guard", () => {
