...
| Permission | Purpose |
|---|---|
| `sec1.audit_event.ingest` | Internal event ingestion; source-bound identity only |
...
| `sec1.audit_event.verify_integrity` | Verify hash-chain and external seal |
...
| `sec1.incident_break_glass.read` | Emergency read-only incident audit access |
| `sec1.seal_anchor.manage` | Manage external seal anchoring |
| `sec1.recovery.verify` | Verify restored audit store integrity |
...
sec1.external_seal.disable
sec1.source_module.spoof
sec1.backfill.freeform
sec1.restore.authoritative_without_verification
```
...
9. External seal anchor configuration.
10. Incident break-glass audit read.
11. Restore-to-authoritative decision.
...
8. Audit correction creator cannot be original event actor/subject.
9. SEC-01 audit operator cannot also control external seal root.
10. Incident break-glass reader cannot approve own emergency access.
11. Recovery operator cannot solely approve restore as authoritative.
...
## 8. IAM-02 Protected-Action Registration

All SEC-01 sensitive permissions/actions must be registered in IAM-02 protected-action registry.

Minimum protected actions:

1. `sec1.audit_event.read_sensitive`
2. `sec1.audit_event.search_sensitive`
3. `sec1.evidence_export.request`
4. `sec1.evidence_export.approve`
5. `sec1.evidence_export.download`
6. `sec1.monitoring_rule.manage`
7. `sec1.event_schema.manage`
8. `sec1.audit_correction.create`
9. `sec1.incident_break_glass.read`
10. `sec1.seal_anchor.manage`
11. `sec1.recovery.verify`
12. `sec1.retention.manage`

Rules:

1. Unregistered SEC-01 sensitive action fails closed under IAM-02.
2. IAM-02 SoD applies to SEC-01 export, correction, break-glass, and seal control.
3. Emergency audit read uses IAM-02 break-glass pattern and is not audit bypass.

---

## 9. Ingestion Identity Rules

1. Ingestion service account is scoped to one source module or approved stream set.
2. Payload `source_module` must match authenticated ingestion identity.
3. Cross-module ingestion is blocked.
4. Source emission sequence is mandatory for sensitive source streams.
5. Missing source emission sequence for sensitive stream fails ingestion or creates Critical alert according to policy.
