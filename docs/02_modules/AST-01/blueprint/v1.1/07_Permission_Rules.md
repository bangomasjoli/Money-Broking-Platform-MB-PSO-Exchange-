# AST-01 — 07 Permission Rules (v1.1)
...
5. Service identities are allow-listed per **subject**; an unlisted subject is refused before any state is read.
...
| `ast1.classification.approve_elevated` | `MLRO` | Second checker on the `elevated` path |
...
| `ast1.lineage.merge_request` / `.approve` | compliance / `COMPLIANCE_OFFICER` | Irreversible |
