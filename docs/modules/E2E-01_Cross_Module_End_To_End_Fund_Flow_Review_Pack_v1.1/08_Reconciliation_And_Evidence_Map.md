...
## 4. v1.1 Per-Correlation Evidence Bundle

Every transaction must be retrievable by `correlation_id`.

Bundle must include:

1. initiating request.
2. point-in-time eligibility snapshot.
3. CFG decisions.
4. IAM/maker-checker/dual-auth decisions.
5. CLT/KYC/AML status evidence.
6. WLT decision/source/destination evidence.
7. LED reservation/hold/settlement/journal/hash-chain refs.
8. TRD quote/LP/fill/conservation evidence where applicable.
9. external rail/LP/custodian references.
10. SEC expected event manifest.
11. SEC emitted events.
12. reconciliation outputs.
13. exception/compensation records.
14. final client-facing confirmation.

Retention:

```txt
evidence_bundle_retention = longest_applicable_statutory_or_regulatory_retention
```

## 5. v1.1 Expected-vs-Emitted SEC Event Reconciliation

For every correlation ID, E2E-01 must define an expected event manifest.

Example trade manifest:

1. quote requested.
2. LP quote received.
3. client quote created.
4. quote accepted.
5. CFG revalidated.
6. AML gate verified.
7. LED hold created.
8. LP execution submitted.
9. LP fill received.
10. fill conservation checked.
11. settlement handoff sent.
12. LED settlement outcome.
13. client confirmation.
14. reconciliation closed.

Missing required event = Critical break.
