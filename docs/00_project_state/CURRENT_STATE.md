...
- Migration head **071** in the repository (`platform/infra/migrations/071_cfg1_environment_scope.cjs`, 71 migrations — added by `MIG-004`, **not yet accepted**). The last accepted head is **070** (`070_fnd_rate_limit_policy_privilege_hardening.cjs`), also stated in [OPEN_FINDINGS.md](../OPEN_FINDINGS.md) (FND-01 Shared Rate-Limit Engine).
...
**MIG-004** (CFG-01 `environment_scope` + live decision-chain step 4) — **IMPLEMENTED / NOT ACCEPTED**: awaiting independent review and human acceptance (see [03-evidence.md](../03_implementation/tasks/MIG-004/03-evidence.md) for the implementation commit and DB-backed evidence). `CFG-FIND-001` and the CFG-01 part of `FND-FIND-013` are implemented in code but **remain OPEN until acceptance**; `CFG-FIND-002` untouched. New observation MIG-004-O1 (pre-existing CLT-01 test-isolation race) recorded for triage in [02-implementation-report.md](../03_implementation/tasks/MIG-004/02-implementation-report.md). Plan approved by Aiman, 2026-09-25. Records: [tasks/MIG-004/](../03_implementation/tasks/MIG-004/). Human decisions HD-1…HD-4 all **APPROVED**: `current_state` = product/operational activation conjunct, recorded as **DEC-014**; canonical PRODUCTION denies `production_activation_absent` until production-activation state exists; the `feature` config-integrity seal is left unextended in MIG-004 and its decision-time verification gap is tracked separately as **`CFG-FIND-002`** (MEDIUM); a structural `exchange.` prefix deny is added at evaluation. Decision precedence corrected: prohibited-registry deny precedes the caller-environment-mismatch check, so a permanently prohibited request always denies as `prohibited` even with a mismatched environment. Those two findings close only on independent review + human acceptance of `MIG-004`, not on implementation.

Before it, **MIG-005** (canonical five-environment model in `@aix/foundation`; `demo` added; `staging` and unknown → PRODUCTION) is **ACCEPTED** at `5f78a0b`, with controlled carry-forwards. The review ran in the same session as the implementation; the human accepted knowing this. Records: [tasks/MIG-005/](../03_implementation/tasks/MIG-005/). Carry-forwards: `FND-FIND-012`, `FND-FIND-013`, `WLT-FIND-016`, plus governance cleanup G-1 (record the staging mapping as decided in the masters, plus the Doc 00 §25.2 `MIG-004` wording, R-5). No capability enabled, no migration added.
...
- **Governance / masters:** the `DEC-013` core re-baseline is complete (§7A). The stated next phase is **module blueprints**: create `EXM-01`, `EXO-01`, `EXC-01`, `EXP-01`; update the blueprints Doc 00 §25.2 lists as affected. **The re-baseline authorises no application code**; the `MIG-001`…`MIG-010` migrations each require their own approved task record. `MIG-005` is **ACCEPTED** (`5f78a0b`); `MIG-004` is **IMPLEMENTED / NOT ACCEPTED** ([tasks/MIG-004/](../03_implementation/tasks/MIG-004/)); next is its independent review. Nothing else is started.
