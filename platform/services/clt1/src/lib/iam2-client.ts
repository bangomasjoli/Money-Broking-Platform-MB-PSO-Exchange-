/**
 * CLT-01 -> IAM-02 HTTP client (Phase 2). The ONLY way CLT-01 may consume IAM-02's permission
 * guard or approval/execute-verify surface — never import services/iam2/src/** (F3(c)
 * module-import boundary). This is CLT-01's own fresh copy of CFG-01's identical pattern
 * (services/cfg1/src/lib/iam2-client.ts, itself modelled on SEC-01's) — not imported from either,
 * F3(c) applies between every pair of service directories.
 *
 * Wraps two IAM-02 routes:
 *   - `POST /internal/iam2/permission/check` — baseline permission decision (advisory-only,
 *     never itself authorises a mutation). Used by review/reject/hold/outcome-status-read and by
 *     approve/request's own baseline check.
 ...
 *     (services/clt1/src/routes/decisions.ts) — the sole CLT-01 action requiring a real
 ...
 * `022_iam2_register_clt1_permissions.cjs`) — by design, so the catalogue itself documents which
 * action is approval-sensitive. Treating this the same as a real `deny` would make the BASELINE
 * check permanently unsatisfiable for `approve/request` — the exact CFG-01 Phase 3A F-1 shape,
 * applied here from day one rather than discovered by a later review pass. The REAL gate for
 * `approve` was always `verifyDecisionToken`/execute-verify (§ above), never this baseline call —
 * this baseline call exists only to catch genuine denials (`deny`/`licence_locked`) and IAM-02
 * unavailability before wasting an approval cycle.
 *
 * Dependency injection for tests: `fetchImpl` lets a test stub this call instead of requiring a
 * live IAM-02 service (Clt1Config.iam2FetchImpl — never populated from env, test-only) — mirrors
 * Cfg1Config.iam2FetchImpl exactly.
 */
...
export interface CheckPermissionInput {
  actorId: string;
  sessionId?: string;
  /** Fully-qualified iam2.permission.permission_code, e.g. "clt1.application.approve". */
  action: string;
  resource: string;
  entityId?: string;
  clientId?: string;
...
/** Decisions that PASS CLT-01's baseline check — see this file's header comment for why
 ...
 * Used by CLT-01's `approve/apply` route to verify+consume a decision token a maker-checker
 * approval flow already minted via IAM-02's EXISTING generic `/iam2/approvals/request` +
 * `/iam2/approvals/:id/approve` endpoints (run by an operator OUTSIDE CLT-01's own routes, exactly
 * as CFG-01/SEC-01's own precedent) — no new IAM-02 code, no IAM-02 guard change.
 *
 ...
 * surfaced when parseable, purely for diagnostics — CLT-01's `approve/apply` route collapses ALL
 * of them to a single generic `CLT1_APPROVAL_REQUIRED`, same discipline CFG-01/SEC-01 already
 * apply.
 */
export interface VerifyDecisionTokenInput {
   ...
   * issues the token with `actorUserId: approval.maker_user_id`). CLT-01 passes the stored
   * `application_decision_request` row's own `requested_by` value here, never a caller-presented
   * arbitrary actor. */
  actorId: string;
  sessionId?: string;
  ...
  entityId?: string;
  clientId?: string;
  /** CLT-01's own recomputed `fingerprint()` of the decision request's apply payload — must match
