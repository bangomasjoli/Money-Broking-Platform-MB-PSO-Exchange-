...
v1.1 resolved:
1. Atomic live-balance reservation with per-account serialisation.
2. Snapshot as reconciliation-only artefact.
3. Two-leg linked DvP settlement and partial-settlement unwind.
4. Preventive safeguarding with free vs encumbered backing.
5. FX/cross-asset conversion as linked A-out/B-in legs.
6. Bounded rounding/residual policy and zero AIX net position.
7. Journal hash-chain, independent verification and external anchoring.
8. Hold pinning while settlement is in-flight.
9. Post-credit clawback for chain reorg / fiat recall.
10. Idempotency same-key/different-payload conflict rejection.
11. Operational/clearing/suspense/residual account bounds.
12. Fee and settlement atomicity / compensated saga.
13. Per-movement and intraday safeguarding checks.
